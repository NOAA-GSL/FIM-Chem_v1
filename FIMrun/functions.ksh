...
function get_patternfile
{
  # If a pattern data file for the current year exists, use it. Otherwise, use the
  # one in $datadir.

  test -z "$yyyymmddhhmm" && fail "$0: yyyymmddhhmm not set"
  patfile=$DATADIR/pattern_${yyyymmddhhmm:5:4}.dat
  test -f $patfile || patfile=$PREP/patterns_icos.dat
}
...
function get_nl_value_nf
{
  # Assigns to the specified variable the value corresponding to the given
  # namelist file, namelist and key.
  #
  # usage: get_nl_value_nf namelist_file namelist key variable
