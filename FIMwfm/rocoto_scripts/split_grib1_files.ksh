...
##    added WEASD inventory                      Sept 2015
#############################################################
...
##       weasd.inv.$FCST  -- snow water equivalent field
##
##    inputs
##      fcst      - forecast hour
##      INV_CLD   - generate inventory for CWAT, OLR if set to T
...
create_grib1_inv()
{
  ...
  if [ "$inv_w2" == "T" ]; then
    ...
    w2flds="PWAT|MSLMA"
    egrep "($mandatory_levels)" prs.inv.$fcst | egrep -v "($ignvars)" > w2flds.inv.$fcst
  ...
  if [ "$inv_ncl" == "T" ]; then
    ...
    fimx_sfcflds="MSLET|4LFTX|LFTX|KX|SX|MCONV"
    egrep "($other_levels)" prs.inv.$fcst | egrep -v "($ignvars)" > ncl.inv.$fcst
    ...
    ## create separate inventory for snow water equivalent; 
    ##    grib2 field for snow water equivalent gets encoded incorrectly when converted from grib1, so use grib1 for now
    egrep "WEASD" ncl.inv.$fcst > weasd.inv.$fcst
  fi
  ...
  do
    ...
    if [ -s $invFile ]; then
      if [[ "$type" != "weasd" ]]; then
        ...
        wgrib -i -grib -o ${gribFile}.${type} $gribFile < $invFile
      fi
    fi
  done
    ...
    if [[ "$file" != *weasd* ]]; then
      ...
      if [[ $status -eq 0 ]]; then
        if [[ ${fcst} -gt 252 ]]; then
          ...
          status=$?
          if [[ $status -ne 0 ]]; then
            echo timetamp_fix_grib2 not executed!!
            exit 1
          fi
        fi
