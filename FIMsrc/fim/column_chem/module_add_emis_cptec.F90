...
CONTAINS
       subroutine add_emis_cptec(dtstep,ktau,dz8w,      &
            rho_phy,chem,num_chem,num_emis_ant,hour,                    &
...
!   TYPE(grid_config_rec_type),  INTENT(IN   )    :: config_flags
   ...
   INTEGER,      INTENT(IN   ) :: julday,num_chem,num_emis_ant,hour,            &
                                  ids,ide, jds,jde, kds,kde,               &
                                  ims,ime, jms,jme, kms,kme,               &
                                  its,ite, jts,jte, kts,kte
   INTEGER,      INTENT(IN   ) ::                                          &
   ...
   REAL, DIMENSION( ims:ime, kms:kemit, jms:jme,num_emis_ant ),            &
         INTENT(IN ) ::                                                    &
                                         emis_ant
   REAL, DIMENSION(  jms:jme,num_emis_ant ),            &
         INTENT(INOUT ) ::                                                    &
                                         emiss_ab1
!
!
...
!      REAL(KIND=8), INTENT(IN   ) :: curr_secs
    ...
    integer :: i,j,k
    real :: time,conv_rho,nlong
    integer :: iweek,idays
    real :: tign,timeq,r_q,r_antro
    real, dimension(7) :: week_CYCLE
    integer :: century_year,month,day,minute,second,ten_thousandth 
...
!         second,ten_thousandth)
    ...
    !itime1=hour
    itime1=MOD(gmt+int(nlong/15)+24.,24.)
    time=(ktau-1)*dtstep
...
!       endif
        ...
        chem(i,k,j,p_so2)  = chem(i,k,j,p_so2)                         &
          ...
          emiss_ab1 (j,p_e_so2)=emis_ant(i,k,j,p_e_so2)*conv_rho
        ...
        chem(i,k,j,p_no2)   = chem(i,k,j,p_no2)                          &
          ...
          emiss_ab1 (j,p_e_no2)=emis_ant(i,k,j,p_e_no2)*conv_rho
...
!                         +emis_ant(i,k,j,p_e_ol2)*conv_rho
        ...
        !chem(i,k,j,p_pm_25)  =  chem(i,k,j,p_p25)                        &
        !                 +r_antro*emis_ant(i,k,j,p_e_pm_25)/rho_phy(i,k,j)/dz8w(i,k,j)*dtstep
        !chem(i,k,j,p_pm_10)  =  chem(i,k,j,p_p10)                        &
