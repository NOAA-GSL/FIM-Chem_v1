...
!  USE module_data_radm2
  ...
  USE module_wrf_control, only:num_chem,num_moist 
...
CONTAINS
   ...
   SUBROUTINE  soa_vbs_driver ( ktau,dtstep,t_phy,moist,aerwrf,p8w,  &
               t8w,alt,p_phy,chem,rho_phy,dz8w,rh,z_at_w,             & 
...
!!! TUCCELLA
         ...
         pm2_5_dry_ec(i,k,j) = pm2_5_dry_ec(i,k,j)+chem(i,k,j,p_ecj) &
                               + chem(i,k,j,p_eci)
         ...
         pm2_5_dry_ec(i,k,j) = pm2_5_dry_ec(i,k,j) / alt(i,k,j)
         pm2_5_water(i,k,j)  = pm2_5_water(i,k,j) / alt(i,k,j)
      enddo
      enddo
      enddo
      do j=jts,jte
         ...
         do k=kts,kte
            do i=its,ite
               ...
               pm10(i,k,j) = pm2_5_dry(i,k,j)                       &
                           + ( chem(i,k,j,p_antha)               &
                           + chem(i,k,j,p_soila)                 &
                           + chem(i,k,j,p_seas) ) / alt(i,k,j)
!!!TUCCELLA
               if( p_p25cwi .gt. p_p25i) then
                    pm10(i,k,j) = pm10(i,k,j)                       &
                           + ( chem(i,k,j,p_anthcw)               &
                           + chem(i,k,j,p_soilcw)                 &
                           + chem(i,k,j,p_seascw) ) / alt(i,k,j)
               endif
            enddo
         enddo
      enddo
...
SUBROUTINE     soa_vbs_depdriver ( & !id,config_flags,ktau,dtstep,                        &
               ust,t_phy,moist,p8w,rmol,znt,pbl,                        &
...
!        if (aer_drydep_opt == 11) then  !lzhang
...
!minit= 1.4*( cblk(lcell,vorgpaj) + cblk(lcell,vorgpai) ) ! exclude EC from absorbing mass
 minit= ( cblk(lcell,vorgpaj) + cblk(lcell,vorgpai) ) ! exclude EC from absorbing mass  !lzhang
...
!
!       RSURFQ=max(RSURFQ,50.)
           if (RSURFQ==0.) then  !lzhang
           SUM0=SUM0+WGAUS(N)*(VSEDQ )  ! Quadrature sum for 0 moment
           SUM3=SUM3+WGAUS(N)*(VSEDQ )*DQ**3  ! Quadrature sum for 3rd moment
           else
           SUM0=SUM0+WGAUS(N)*(VSEDQ + 1./RSURFQ)  ! Quadrature sum for 0 moment
           SUM3=SUM3+WGAUS(N)*(VSEDQ + 1./RSURFQ)*DQ**3  ! Quadrature sum for 3rd moment
           endif
...
!!! TUCCELLA (BUG, commented the line below)
   ...
   USE module_initial_chem_namelists,only:AER_IC_DEFAULT
   implicit none
...
!
        if(chem_in_opt == 1 .or. readrestart ) return
        ...
        do l=p_so4aj,p_eci
        chem(ims:ime,kms:kme,jms:jme,l)= 0.1e-3
        enddo
        chem(ims:ime,kms:kme,jms:jme,p_p25i)=1.
        chem(ims:ime,kms:kme,jms:jme,p_p25j)=1.
...
!Option for alternate ic's
        if( aer_ic_opt == AER_IC_DEFAULT ) then
          chem(i,k,j,p_so4aj)=chem(i,k,j,p_sulf)*CONVFAC(i,k,j)*MWSO4*splitfac*so4vaptoaer
          chem(i,k,j,p_so4ai)=chem(i,k,j,p_sulf)*CONVFAC(i,k,j)*MWSO4*(1.-splitfac)*so4vaptoaer
...
!-----------------------------------------------------------------------
  implicit none
   ...
   real, intent(in) :: dcen_sect( maxd_asize, maxd_atype )
       ...
       ! scav rates for aerosol number and volume are pre-calculated for a range
       ! of volume-mean diameters
       !    as well as aerosol densities and air pressure and temperature
       ! for specific time/location/aerosol properties, these rates are
       ! interpolated, and
       !    the size interpolation uses [(actual volume-mean diameter)/(base
...
!
        ...
        if (nnfit .gt. nnfit_maxd) then
            print *, '*** subr. aerosol_impact_setup -- nnfit too big'
            stop
        end if
...
SUBROUTINE soa_vbs_addemiss(  dtstep, u10, v10, alt, dz8w, xland, chem,                      &
                             ...
                             dust_opt,ktau,p8w,u_phy,v_phy,rho_phy,g,area,erod,                   &
...
!------------------------------------------------------------------------
!  USE module_wrf_control, only: num_ebu, num_chem,num_emis_ant
!  USE module_initial_chem_namelists
  !USE module_state_description, only:  num_chem

  INTEGER, INTENT(IN   )   ::    seasalt_emiss_active,kemit,emissopt,   &
                                 dust_emiss_active,num_soil_layers,     &
                                 ktau,dust_opt,biom,                    &
                                 num_ebu, num_chem,num_emis_ant,        &
                                 ids,ide, jds,jde, kds,kde,             &
                                 ims,ime, jms,jme, kms,kme,             &
...
! 1/(dry air density) and layer thickness (m)
  ...
  REAL,  DIMENSION( ims:ime , jms:jme ),                                &
       INTENT(IN   ) ::                                                 &
       clayfrac,sandfrac,u10, v10, xland, slai, ust,area,vegfra,znt,gsw
...
! Aitken mode first...
  ...
  chem(its:ite,kts:kemit,jts:jte,p_nu0) =                        &
       ...
       1.4*orgfac*emis_ant(its:ite,kts:kemit,jts:jte,p_e_orgi) )
...
! Accumulation mode next...
  ...
  chem(its:ite,kts:kemit,jts:jte,p_ac0) =                        &
       ...
       1.4*orgfac*emis_ant(its:ite,kts:kemit,jts:jte,p_e_orgj) )
...
! Aitken mode first...
  ...
  chem(its:ite,kts:kemit,jts:jte,p_nu0) =                        &
       ...
       1.4*orgfac*.25*emis_ant(its:ite,kts:kemit,jts:jte,p_e_oc) )
...
! Accumulation mode next...
  ...
  chem(its:ite,kts:kemit,jts:jte,p_ac0) =                        &
       ...
       1.4*orgfac*.75*emis_ant(its:ite,kts:kemit,jts:jte,p_e_oc) )
...
!
  ...
  chem(its:ite,kts:kemit,jts:jte,p_p25j) =                        &
       chem(its:ite,kts:kemit,jts:jte,p_p25j) +                   &
       .75*emis_ant(its:ite,kts:kemit,jts:jte,p_e_pm_25)*factor(its:ite,kts:kemit,jts:jte)

  chem(its:ite,kts:kemit,jts:jte,p_p25i) =                        &
       chem(its:ite,kts:kemit,jts:jte,p_p25i) +                   &
       .25*emis_ant(its:ite,kts:kemit,jts:jte,p_e_pm_25)*factor(its:ite,kts:kemit,jts:jte)

  chem(its:ite,kts:kemit,jts:jte,p_orgpaj) =                     &
       chem(its:ite,kts:kemit,jts:jte,p_orgpaj) +                &
       1.4*.75*emis_ant(its:ite,kts:kemit,jts:jte,p_e_oc)*factor(its:ite,kts:kemit,jts:jte)

  chem(its:ite,kts:kemit,jts:jte,p_orgpai) =                     &
       chem(its:ite,kts:kemit,jts:jte,p_orgpai) +                &
       1.4*.25*emis_ant(its:ite,kts:kemit,jts:jte,p_e_oc)*factor(its:ite,kts:kemit,jts:jte)
...
! Aitken mode first...
  ...
  chem(its:ite,kts:kte,jts:jte,p_nu0) =                        &
       ...
       1.4*orgfac*.25*ebu(its:ite,kts:kte,jts:jte,p_ebu_oc) )
...
! Accumulation mode next...
  ...
  chem(its:ite,kts:kte,jts:jte,p_ac0) =                        &
      ...
      .75*ebu(its:ite,kts:kte,jts:jte,p_ebu_bc) ) +                  &
       1.4*orgfac*.75*ebu(its:ite,kts:kte,jts:jte,p_ebu_oc) )
! coarse
...
!
  ...
  chem(its:ite,kts:kte,jts:jte,p_orgpaj) =                     &
       chem(its:ite,kts:kte,jts:jte,p_orgpaj) +                &
       1.4*.75*ebu(its:ite,kts:kte,jts:jte,p_ebu_oc)*factor(its:ite,kts:kte,jts:jte)

  chem(its:ite,kts:kte,jts:jte,p_orgpai) =                     &
       chem(its:ite,kts:kte,jts:jte,p_orgpai) +                &
       1.4*.25*ebu(its:ite,kts:kte,jts:jte,p_ebu_oc)*factor(its:ite,kts:kte,jts:jte)
  ...
  if( dust_opt == 1 ) then
    ...
    if( dust_opt == 3 ) then
      call soa_vbs_dust_afwaemis(                                &
           ktau,dtstep,num_soil_layers,alt,u_phy,                 &
           v_phy,chem,rho_phy,dz8w,smois,u10,v10,p8w,erod,        &
           ivgtyp,isltyp,vegfra,xland,gsw,area,g,                 &
           ust,znt,clayfrac,sandfrac,dust_alpha,dust_gamma,num_chem,                &
           ids,ide, jds,jde, kds,kde,                             &
           ims,ime, jms,jme, kms,kme,                             &
           its,ite, jts,jte, kts,kte                              )
  end if
...
!   Loop over i,j and apply seasalt emissions
    ...
    !Skip this point if over land. xland=1 for land and 0 for water. !lzhang
...
!----------------------------------------------------------------------
   ...
   subroutine soa_vbs_dust_emiss(  slai,ust, smois, ivgtyp, isltyp,         &
                dtstep, u10, v10, alt, dz8w, xland, num_soil_layers,    &
               chem,                                                       &
               ids,ide, jds,jde, kds,kde,                                  &
               ims,ime, jms,jme, kms,kme,                                  &
               its,ite, jts,jte, kts,kte                                   )
...
! don't do dust over water!!!
     ...
     airmas=-(p8w(i,kts+1,j)-p8w(i,kts,j))*area(i,j)/g   ! kg 
...
!     dxy=dx*dx
      ...
      gwet=smois(i,1,j)/maxsmc(isltyp(i,j))
...
END SUBROUTINE soa_vbs_source_du
  ...
  subroutine soa_vbs_dust_afwaemis(ktau,dt,num_soil_layers,alt,u_phy,        &
         v_phy,chem,rho_phy,dz8w,smois,u10,v10,p8w,erod,                   &
         ivgtyp,isltyp,vegfra,xland,gsw,area,g,       &
         ust,znt,clay,sand,alpha,gamma,num_chem,                 &
         ids,ide, jds,jde, kds,kde,                                        &
         ims,ime, jms,jme, kms,kme,                                        &
         its,ite, jts,jte, kts,kte                                         )
 ! USE module_data_gocart_dust
...
!lzhang
  ...
  real*8  cdustdens,jdustdens,mass1part,jdustdiam,cdustdiam,dp_meanvol_tmp
  real dttt
  real*8  soilfacj,rhosoilj,rhosoilc
  real*8  totalemis,accfrac,corfrac,rscale1,rscale2
...
! print *, "ustar before call=",ustar(1,1)
      call soa_vbs_source_dust(imx, jmx, lmx, nmx, smx, dt, tc, ustar, massfrac, &
    ...
    !sum up the dust emission from 0.1-10 um in radius 
    ! unit change from kg/timestep/cell to ug/m2/s
    totalemis=(sum(bems(1:5))/dt)*converi/dxy(1)
     ! to account for the particles larger than 10 um radius
     ! based on assumed size distribution
    jdustemis = totalemis*accfrac   ! accumulation mode
    cdustemis= totalemis*corfrac   ! coarse mode 

         cdustcon = sum(bems(1:5))*corfrac/airmas(1,1)  ! kg/kg-dryair
         cdustcon = cdustcon * converi   ! ug/kg-dryair
         jdustcon = sum(bems(1:5))*accfrac/airmas(1,1)  ! kg/kg-dryair
         jdustcon = jdustcon * converi   ! ug/kg-dryair
         chem(i,k,j,p_p25j)=chem(i,k,j,p_p25j) + jdustcon
         chem(i,k,j,p_soila)=chem(i,k,j,p_soila) + cdustcon
...
end subroutine soa_vbs_dust_afwaemis

  SUBROUTINE soa_vbs_source_dust(imx, jmx, lmx, nmx, smx, dt1, tc, ustar, massfrac,&
...
! calculate total dust emission (tot_emit), taking into account erodibility. 
 ...
 DO n = 1, smx
   ...
   DO i = 1,imx
     ...
     ! that the
     ! lofting efficiency decreases.  Otherwise, super sandy zones would be huge
     ! dust
     ! producers, which is generally not the case.  Equation derived from
     ! wind-tunnel 
...
!
! In calculating the Kok distribution, we assume upper and lower limits to each
! bin.
...
END SUBROUTINE soa_vbs_source_dust
