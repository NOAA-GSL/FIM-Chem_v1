...
USE module_tracer_pointers_500
USE module_gocartracm_pointers
USE module_racmsoavbs_pointers
USE module_data_gocart_dust
USE module_data_gocart_seas
USE module_wrf_control, only: numgas,num_chem,num_emis_ant,num_emis_vol
!
...
if(chem_opt.eq.500)then
   if(num_chem.ne.5)then
      write(6,*) ' num_chem is not equal 5 '
      stop
   endif
   if(num_emis_ant.lt.5)then
      write(6,*) ' num_emis_ant smaller than 5 '
      stop
   endif
   call tracer_pointers_500
   p_qv=1
   p_qc=2
   p_qi=3
endif
...
if(chem_opt.eq.301)then
   if(num_chem.ne.66)then
      write(6,*) ' num_chem is not equal 66 '
      stop
   endif
   if(num_emis_ant.lt.25)then
      write(6,*) ' num_emis_ant smaller than 25 '
      stop
   endif
...
if(chem_opt.eq.108)then
   if(num_chem.ne.103)then
      write(6,*) ' num_chem is not equal 103 '
      stop
 endif
   if(num_emis_ant.lt.25)then
      write(6,*) ' num_emis_ant smaller than 25 '
