...
USE module_chemvars, only:epsilc, pm2_5_dry,pm2_5_dry_ec,pm2_5_water
!USE module_chem_namelist_defaults
USE module_initial_chem_namelists ! ,only:p_bc1,p_oc1,p_sulf,p_e_bc,p_e_oc,p_e_sulf
USE module_aerosols_soa_vbs, only:aerosols_soa_vbs_init,aerosols_soa_vbs_init_aercld_ptrs
CONTAINS
     subroutine chem_prep_fim(ktau,dtstep,rh3d,tr3d,tk3d,st3d,sm3d,dp3d,mp3d,&
             ...
             nvl_gocart, ttday,tcosz,gmt,julday,dtstepc,ph3d,area,ero1,&
...
INTEGER,      INTENT(IN   ) :: num_ebu_in,num_soil_layers,num_chem,numgas,num_moist,julday,     &
...
!  real,dimension(ims:ime,kms:kme,jms:jme),intent(in)::t8w ! change to 'intent(out)' if/when actually set
! .. Intrinsic Functions ..
...
!        if (slmsk2d(j).gt.0.)write(6,*)j,slmsk2d(j)
       ...
       jmaxi=0
       k=1
       if(p_bc2 .gt. 1 .or. chem_opt == 108)then  ! "regular" chem options
       do i=its,ite
       do j=jts,jte
         k=1
...
!           if(chem_opt > 0 ) then
           do j=jts,jte
              do k=kts,kte
                 ...
                 if (chem_opt == 301.or.chem_opt==108) then  !added o3 background !lzhang
                   kk=min(k,kte)
                   !add initial constant into O3,CH4 and CO ect.
                   chem(i,k,j,p_o3)=epsilc
                   maxth=min(400.,th_pvsrf(j))
                   if(tr3d(kk,j,1).gt.maxth) then 
          ...
          if (ktau<=1.and.(chem_opt == 301.or.chem_opt==108)) then  !added GFS o3 background above 380K!lzhang
             do i=its,ite
                do k=kts,kte+1
                   kk=min(k,kte)
                   maxth=min(400.,th_pvsrf(j))
                   if (tr3d(kk,j,1) .GE. maxth ) then
                   do j=jts,jte
                     chem(i,k,j,p_o3)=(airmw/48.)*tr3d(kk,j,4)*1e6 !convert kg/kg to ppm
                   enddo
                   endif !380K
      ...
      if (ktau<=1.and.chem_opt == 108) then
...
if (first_init .and. chem_opt == 108) then
           first_init = .false.
           call aerosols_soa_vbs_init(chem,convfac,z_at_w,           &
               pm2_5_dry,pm2_5_water,pm2_5_dry_ec,                   &
               chem_in_opt, aer_ic_opt,                              &
...
!
      !if (.NOT. readrestart) then
       if(call_gocart.eq.1.and.chem_opt==300)then
       do i=its,ite
       do j=jts,jte
          do k=kts,kte
            do ll=2,nvl_gocart
...
!                 endif
                   do n=1,ndystep
                      ...
                      if(cosszax(1,1).gt.0.)ttday(i,j)=ttday(i,j)+dtstep
                    enddo
