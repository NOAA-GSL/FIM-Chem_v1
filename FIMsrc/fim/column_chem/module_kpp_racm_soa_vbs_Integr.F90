...
! Time                 : Wed May 25 17:16:52 2016
! Working directory    : /scratch3/BMC/wrf-chem/lzhang/WRF/v3.8/WRFV3/chem/KPP/mechanisms/racm_soa_vbs
...
MODULE racm_soa_vbs_Integrator
...
SUBROUTINE  racm_soa_vbs_INTEGRATE( TIN, TOUT, &
...
!!   USE racm_soa_vbs_Global
   ...
   CALL racm_soa_vbs_Rosenbrock(VAR, FIX, RCONST, TIN,TOUT,   &
         ATOL,RTOL,               &
...
END SUBROUTINE  racm_soa_vbs_INTEGRATE

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE  racm_soa_vbs_Rosenbrock(Y, FIX, RCONST, Tstart,Tend, &
           AbsTol,RelTol,            &
           RCNTRL,ICNTRL,RSTATUS,ISTATUS,IERR)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE racm_soa_vbs_Parameters
!!  USE racm_soa_vbs_LinearAlgebra
  IMPLICIT NONE
...
!~~~>  The particular Rosenbrock method chosen
   ...
   ELSE
      ...
      CALL racm_soa_vbs_ros_ErrorMsg(-2,Tstart,ZERO,IERR)
      RETURN
   END IF
...
!~~~>   The maximum number of steps admitted
   ...
   ELSE
      ...
      CALL racm_soa_vbs_ros_ErrorMsg(-1,Tstart,ZERO,IERR)
      RETURN
   END IF

!~~~>  Unit roundoff (1+Roundoff>1)
   Roundoff = racm_soa_vbs_WLAMCH('E')
...
!~~~>  Lower bound on the step size: (positive value)
   ...
   ELSE
      ...
      CALL racm_soa_vbs_ros_ErrorMsg(-3,Tstart,ZERO,IERR)
      RETURN
   END IF
...
!~~~>  Step size can be changed s.t.  FacMin < Hnew/Hexit < FacMax
   ...
   ELSE
      ...
      CALL racm_soa_vbs_ros_ErrorMsg(-4,Tstart,ZERO,IERR)
      RETURN
   END IF
...
!~~~>  Check if tolerances are reasonable
    DO i=1,UplimTol
      IF ( (AbsTol(i) <= ZERO) .OR. (RelTol(i) <= 10.0_dp*Roundoff) &
        ...
        CALL racm_soa_vbs_ros_ErrorMsg(-5,Tstart,ZERO,IERR)
        RETURN
      END IF
    END DO
...
!~~~>   Initialize the particular Rosenbrock method
   SELECT CASE (Method)
     CASE (1)
       CALL racm_soa_vbs_Ros2(ros_S, ros_A, ros_C, ros_M, ros_E,   &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (2)
       CALL racm_soa_vbs_Ros3(ros_S, ros_A, ros_C, ros_M, ros_E,   &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (3)
       CALL racm_soa_vbs_Ros4(ros_S, ros_A, ros_C, ros_M, ros_E,   &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (4)
       CALL racm_soa_vbs_Rodas3(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (5)
       CALL racm_soa_vbs_Rodas4(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE DEFAULT
...
!~~~>  CALL Rosenbrock method
   CALL racm_soa_vbs_ros_Integrator(Y,Tstart,Tend,Texit,      &
        AbsTol, RelTol,                          &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 SUBROUTINE  racm_soa_vbs_ros_ErrorMsg(Code,T,H,IERR)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 END SUBROUTINE  racm_soa_vbs_ros_ErrorMsg

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 SUBROUTINE  racm_soa_vbs_ros_Integrator (Y, Tstart, Tend, T,     &
        AbsTol, RelTol,                          &
!~~~> Rosenbrock method coefficients
        ros_S, ros_M, ros_E, ros_A, ros_C,       &
        ros_Alpha, ros_Gamma, ros_ELO, ros_NewF, &
...
TimeLoop: DO WHILE ( (Direction > 0).AND.((T-Tend)+Roundoff <= ZERO) &
   ...
   IF ( Nstp > Max_no_steps ) THEN  ! Too many steps
      CALL racm_soa_vbs_ros_ErrorMsg(-6,T,H,IERR)
      RETURN
   END IF
   IF ( ((T+0.1_dp*H) == T).OR.(H <= Roundoff) ) THEN  ! Step size too small
      CALL racm_soa_vbs_ros_ErrorMsg(-7,T,H,IERR)
      RETURN
   END IF
...
!~~~>   Compute the function at current time
   CALL racm_soa_vbs_FunTemplate(T,Y,Fcn0, RCONST, FIX, Nfun)

!~~~>  Compute the function derivative with respect to T
   IF (.NOT.Autonomous) THEN
      CALL racm_soa_vbs_ros_FunTimeDeriv ( T, Roundoff, Y, &
                Fcn0, dFdT, RCONST, FIX, Nfun )
   END IF

!~~~>   Compute the Jacobian at current time
   CALL racm_soa_vbs_JacTemplate(T,Y,Jac0, FIX, Njac, RCONST)

!~~~>  Repeat step calculation until current step accepted
UntilAccepted: DO

   CALL racm_soa_vbs_ros_PrepareMatrix(H,Direction,ros_Gamma(1), &
          Jac0,Ghimj,Pivot,Singular, Ndec,  Nsng )
   IF (Singular) THEN ! More than 5 consecutive failed decompositions
       CALL racm_soa_vbs_ros_ErrorMsg(-8,T,H,IERR)
       RETURN
   END IF
...
Stage: DO istage = 1, ros_S
      ...
      ! For the 1st istage the function has been computed previously
       IF ( istage == 1 ) THEN
         CALL racm_soa_vbs_WCOPY(NVAR,Fcn0,1,Fcn,1)
      ! istage>1 and a new function evaluation is needed at the current istage
       ELSEIF ( ros_NewF(istage) ) THEN
         CALL racm_soa_vbs_WCOPY(NVAR,Y,1,Ynew,1)
         DO j = 1, istage-1
           CALL racm_soa_vbs_WAXPY(NVAR,ros_A((istage-1)*(istage-2)/2+j), &
            K(NVAR*(j-1)+1),1,Ynew,1)
         END DO
         Tau = T + ros_Alpha(istage)*Direction*H
         CALL racm_soa_vbs_FunTemplate(Tau,Ynew,Fcn, RCONST, FIX, Nfun)
       END IF ! if istage == 1 elseif ros_NewF(istage)
       CALL racm_soa_vbs_WCOPY(NVAR,Fcn,1,K(ioffset+1),1)
       DO j = 1, istage-1
         HC = ros_C((istage-1)*(istage-2)/2+j)/(Direction*H)
         CALL racm_soa_vbs_WAXPY(NVAR,HC,K(NVAR*(j-1)+1),1,K(ioffset+1),1)
       END DO
       IF ((.NOT. Autonomous).AND.(ros_Gamma(istage).NE.ZERO)) THEN
         HG = Direction*H*ros_Gamma(istage)
         CALL racm_soa_vbs_WAXPY(NVAR,HG,dFdT,1,K(ioffset+1),1)
       END IF
       CALL racm_soa_vbs_ros_Solve(Ghimj, Pivot, K(ioffset+1), Nsol)
...
!~~~>  Compute the new solution
   CALL racm_soa_vbs_WCOPY(NVAR,Y,1,Ynew,1)
   DO j=1,ros_S
         CALL racm_soa_vbs_WAXPY(NVAR,ros_M(j),K(NVAR*(j-1)+1),1,Ynew,1)
   END DO

!~~~>  Compute the error estimation
   CALL racm_soa_vbs_WSCAL(NVAR,ZERO,Yerr,1)
   DO j=1,ros_S
        CALL racm_soa_vbs_WAXPY(NVAR,ros_E(j),K(NVAR*(j-1)+1),1,Yerr,1)
   END DO
   Err = racm_soa_vbs_ros_ErrorNorm ( Y, Ynew, Yerr, AbsTol, RelTol, VectorTol )
...
!~~~>  Check the error magnitude and adjust step size
   ...
   IF ( (Err <= ONE).OR.(H <= Hmin) ) THEN  !~~~> Accept step
      Nacc = Nacc+1
      CALL racm_soa_vbs_WCOPY(NVAR,Ynew,1,Y,1)
      T = T + Direction*H
...
!~~~> Succesful exit
  ...
  END SUBROUTINE  racm_soa_vbs_ros_Integrator


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  REAL(kind=dp) FUNCTION  racm_soa_vbs_ros_ErrorNorm ( Y, Ynew, Yerr, &
...
! Local variables
   ...
   Err  = SQRT(Err/NVAR)
    ...
    racm_soa_vbs_ros_ErrorNorm = Err

  END FUNCTION  racm_soa_vbs_ros_ErrorNorm


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE racm_soa_vbs_ros_FunTimeDeriv ( T, Roundoff, Y, &
                Fcn0, dFdT, RCONST, FIX, Nfun )
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~> Local variables
   ...
   CALL racm_soa_vbs_FunTemplate(T+Delta,Y,dFdT, RCONST, FIX, Nfun)
   CALL racm_soa_vbs_WAXPY(NVAR,(-ONE),Fcn0,1,dFdT,1)
   CALL racm_soa_vbs_WSCAL(NVAR,(ONE/Delta),dFdT,1)

  END SUBROUTINE  racm_soa_vbs_ros_FunTimeDeriv


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE  racm_soa_vbs_ros_PrepareMatrix ( H, Direction, gam, &
             Jac0, Ghimj, Pivot, Singular, Ndec,  Nsng  )
! --- --- --- --- --- --- --- --- --- --- --- --- ---
...
#ifdef FULL_ALGEBRA    
     CALL racm_soa_vbs_WCOPY(NVAR*NVAR,Jac0,1,Ghimj,1)
     CALL racm_soa_vbs_WSCAL(NVAR*NVAR,(-ONE),Ghimj,1)
     ghinv = ONE/(Direction*H*gam)
     DO i=1,NVAR
       Ghimj(i,i) = Ghimj(i,i)+ghinv
     END DO
#else
     CALL racm_soa_vbs_WCOPY(LU_NONZERO,Jac0,1,Ghimj,1)
     CALL racm_soa_vbs_WSCAL(LU_NONZERO,(-ONE),Ghimj,1)
     ghinv = ONE/(Direction*H*gam)
     DO i=1,NVAR
...
!~~~>    Compute LU decomposition
     CALL racm_soa_vbs_ros_Decomp( Ghimj, Pivot, ising, Ndec )
     IF (ising == 0) THEN
!~~~>    If successful done
        Singular = .FALSE.
     ELSE ! ising .ne. 0
!~~~>    If unsuccessful half the step size; if 5 consecutive fails then return
  ...
  END SUBROUTINE  racm_soa_vbs_ros_PrepareMatrix


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE  racm_soa_vbs_ros_Decomp( A, Pivot, ising, Ndec )
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#ifdef FULL_ALGEBRA    
   CALL  racm_soa_vbs_DGETRF( NVAR, NVAR, A, NVAR, Pivot, ising )
#else   
!!!  use direct adressing in decomp 
!!!    CALL racm_soa_vbs_KppDecomp ( A, ising )
CALL decomp_racm_soa_vbs ( A, ising )
   Pivot(1) = 1
#endif
   Ndec = Ndec + 1

  END SUBROUTINE  racm_soa_vbs_ros_Decomp


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE  racm_soa_vbs_ros_Solve( A, Pivot, b, Nsol )
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#ifdef FULL_ALGEBRA    
   CALL  racm_soa_vbs_DGETRS( 'N', NVAR , 1, A, NVAR, Pivot, b, NVAR, 0 )
#else   
   CALL racm_soa_vbs_KppSolve( A, b )
#endif

   Nsol = Nsol+1

  END SUBROUTINE  racm_soa_vbs_ros_Solve



!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE  racm_soa_vbs_Ros2 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
...
!~~~> Gamma_i = \sum_j  gamma_{i,j}
 ...
 END SUBROUTINE  racm_soa_vbs_Ros2


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE  racm_soa_vbs_Ros3 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
           ros_Gamma,ros_NewF,ros_ELO,ros_Name)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~> Gamma_i = \sum_j  gamma_{i,j}
  ...
  END SUBROUTINE  racm_soa_vbs_Ros3

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE  racm_soa_vbs_Ros4 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
           ros_Gamma,ros_NewF,ros_ELO,ros_Name)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~> Gamma_i = \sum_j  gamma_{i,j}
  ...
  END SUBROUTINE  racm_soa_vbs_Ros4

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE  racm_soa_vbs_Rodas3 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
            ros_Gamma,ros_NewF,ros_ELO,ros_Name)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~> Gamma_i = \sum_j  gamma_{i,j}
  ...
  END SUBROUTINE  racm_soa_vbs_Rodas3

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  SUBROUTINE  racm_soa_vbs_Rodas4 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
             ros_Gamma,ros_NewF,ros_ELO,ros_Name)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!        main and the embedded scheme orders plus 1
  ...
  END SUBROUTINE  racm_soa_vbs_Rodas4

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!   End of the set of internal Rosenbrock subroutines
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
END SUBROUTINE  racm_soa_vbs_Rosenbrock
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE  racm_soa_vbs_FunTemplate( T, Y, Ydot, RCONST, FIX, Nfun )
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   USE racm_soa_vbs_Parameters
!! USE racm_soa_vbs_Global
!! USE racm_soa_vbs_Function
!! USE racm_soa_vbs_Rates
!~~~> Input variables
...
!!   CALL Update_SUN()
!!   CALL Update_RCONST()
   CALL racm_soa_vbs_Fun( Y, FIX, RCONST, Ydot )
!!   TIME = Told

   Nfun = Nfun+1

END SUBROUTINE  racm_soa_vbs_FunTemplate


!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE  racm_soa_vbs_JacTemplate( T, Y, Jcb, FIX, Njac, RCONST )
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!!    CALL Update_RCONST()
#ifdef FULL_ALGEBRA    
    CALL racm_soa_vbs_Jac_SP(Y, FIX, RCONST, JV)
    DO j=1,NVAR
      DO i=1,NVAR
         Jcb(i,j) = 0.0d0
      END DO
    ...
    END DO
#else
    CALL racm_soa_vbs_Jac_SP( Y, FIX, RCONST, Jcb )
#endif   
!!    TIME = Told

    Njac = Njac+1

END SUBROUTINE  racm_soa_vbs_JacTemplate
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
! racm_soa_vbs_Fun - time derivatives of variables - Agregate form
!   Arguments :
...
SUBROUTINE racm_soa_vbs_Fun ( V, F, RCT, Vdot )
...
END SUBROUTINE racm_soa_vbs_Fun

! End of racm_soa_vbs_Fun function
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
! racm_soa_vbs_Jac_SP - the Jacobian of Variables in sparse matrix representation
!   Arguments :
!      V         - Concentrations of variable species (local)
!      F         - Concentrations of fixed species (local)
!      RCT       - Rate constants (local)
...
SUBROUTINE racm_soa_vbs_Jac_SP ( V, F, RCT, JVS )
...
! JVS(9) = Jac_FULL(3,38)
  JVS(9) = 0.37*B(182)
! JVS(10) = Jac_FULL(3,42)
  JVS(10) = 0.05*B(452)+0.039*B(454)
! JVS(11) = Jac_FULL(3,47)
  JVS(11) = 0.01*B(194)
! JVS(12) = Jac_FULL(3,48)
  JVS(12) = 0.28*B(190)
! JVS(13) = Jac_FULL(3,51)
  JVS(13) = 0.15*B(188)
! JVS(14) = Jac_FULL(3,54)
  JVS(14) = 0.11*B(200)
! JVS(15) = Jac_FULL(3,57)
  JVS(15) = 0.45*B(196)
! JVS(16) = Jac_FULL(3,71)
  JVS(16) = 0.11*B(198)
! JVS(17) = Jac_FULL(3,75)
  JVS(17) = 0.14*B(184)
! JVS(18) = Jac_FULL(3,79)
  JVS(18) = 0.37*B(183)+0.14*B(185)+0.15*B(189)+0.28*B(191)+0.01*B(195)+0.45*B(197)+0.11*B(199)+0.11*B(201)+0.039*B(455)
! JVS(19) = Jac_FULL(3,84)
  JVS(19) = 0.036*B(97)+0.05*B(453)
! JVS(20) = Jac_FULL(4,4)
  JVS(20) = 0
! JVS(21) = Jac_FULL(4,42)
  JVS(21) = 0.053*B(454)
! JVS(22) = Jac_FULL(4,47)
  JVS(22) = 0.07*B(194)
! JVS(23) = Jac_FULL(4,49)
  JVS(23) = 0.14*B(462)
! JVS(24) = Jac_FULL(4,55)
  JVS(24) = 0.5*B(359)
! JVS(25) = Jac_FULL(4,56)
  JVS(25) = 0.499*B(361)
! JVS(26) = Jac_FULL(4,60)
  JVS(26) = 0.494*B(365)
! JVS(27) = Jac_FULL(4,64)
  JVS(27) = 0.495*B(355)
! JVS(28) = Jac_FULL(4,65)
  JVS(28) = 0.495*B(357)
! JVS(29) = Jac_FULL(4,69)
  JVS(29) = 0.49*B(363)
! JVS(30) = Jac_FULL(4,71)
  JVS(30) = 0.21*B(198)
! JVS(31) = Jac_FULL(4,73)
  JVS(31) = 0.14*B(186)
! JVS(32) = Jac_FULL(4,75)
  JVS(32) = 0.1*B(184)
! JVS(33) = Jac_FULL(4,76)
  JVS(33) = 0.484*B(384)
! JVS(34) = Jac_FULL(4,77)
  JVS(34) = 0.5*B(382)
! JVS(35) = Jac_FULL(4,78)
  JVS(35) = B(302)+B(343)
! JVS(36) = Jac_FULL(4,79)
  JVS(36) = 0.1*B(185)+0.14*B(187)+0.07*B(195)+0.21*B(199)+0.053*B(455)+0.14*B(463)
! JVS(37) = Jac_FULL(4,80)
  JVS(37) = 0.499*B(353)
! JVS(38) = Jac_FULL(4,81)
  JVS(38) = 0.5*B(380)
! JVS(39) = Jac_FULL(4,82)
...
! JVS(40) = Jac_FULL(4,83)
  JVS(40) = B(340)+B(344)
! JVS(41) = Jac_FULL(4,85)
  JVS(41) = B(299)+B(303)
! JVS(42) = Jac_FULL(4,90)
  JVS(42) = 0.5*B(352)
! JVS(43) = Jac_FULL(5,5)
  JVS(43) = 0
! JVS(44) = Jac_FULL(5,6)
  JVS(44) = 1.075*B(468)
! JVS(45) = Jac_FULL(5,84)
  JVS(45) = 1.075*B(469)
! JVS(46) = Jac_FULL(6,6)
  JVS(46) = -B(468)
! JVS(47) = Jac_FULL(6,7)
  JVS(47) = 1.075*B(466)
! JVS(48) = Jac_FULL(6,84)
  JVS(48) = 1.075*B(467)-B(469)
! JVS(49) = Jac_FULL(7,7)
  JVS(49) = -B(466)
! JVS(50) = Jac_FULL(7,8)
  JVS(50) = 1.075*B(464)
! JVS(51) = Jac_FULL(7,84)
  JVS(51) = 1.075*B(465)-B(467)
! JVS(52) = Jac_FULL(8,8)
  JVS(52) = -B(464)
! JVS(53) = Jac_FULL(8,84)
  JVS(53) = -B(465)
! JVS(54) = Jac_FULL(9,9)
  JVS(54) = 0
! JVS(55) = Jac_FULL(9,10)
  JVS(55) = 1.075*B(474)
! JVS(56) = Jac_FULL(9,84)
  JVS(56) = 1.075*B(475)
! JVS(57) = Jac_FULL(10,10)
  JVS(57) = -B(474)
! JVS(58) = Jac_FULL(10,11)
  JVS(58) = 1.075*B(472)
! JVS(59) = Jac_FULL(10,84)
  JVS(59) = 1.075*B(473)-B(475)
! JVS(60) = Jac_FULL(11,11)
  JVS(60) = -B(472)
! JVS(61) = Jac_FULL(11,12)
  JVS(61) = 1.075*B(470)
! JVS(62) = Jac_FULL(11,84)
  JVS(62) = 1.075*B(471)-B(473)
! JVS(63) = Jac_FULL(12,12)
  JVS(63) = -B(470)
! JVS(64) = Jac_FULL(12,84)
  JVS(64) = -B(471)
! JVS(65) = Jac_FULL(13,13)
  JVS(65) = -B(28)-B(30)
! JVS(66) = Jac_FULL(13,79)
  JVS(66) = B(2)
! JVS(67) = Jac_FULL(14,14)
  JVS(67) = -B(84)
! JVS(68) = Jac_FULL(14,84)
  JVS(68) = -B(85)
! JVS(69) = Jac_FULL(15,15)
  JVS(69) = -B(440)
! JVS(70) = Jac_FULL(15,60)
  JVS(70) = B(284)
! JVS(71) = Jac_FULL(15,84)
  JVS(71) = -B(441)
! JVS(72) = Jac_FULL(15,85)
  JVS(72) = B(285)
! JVS(73) = Jac_FULL(16,16)
  JVS(73) = -B(116)
! JVS(74) = Jac_FULL(16,84)
  JVS(74) = -B(117)
...
! JVS(77) = Jac_FULL(18,18)
  JVS(77) = -B(98)
! JVS(78) = Jac_FULL(18,84)
  JVS(78) = -B(99)
! JVS(79) = Jac_FULL(19,19)
  JVS(79) = -B(80)
! JVS(80) = Jac_FULL(19,88)
  JVS(80) = B(78)
! JVS(81) = Jac_FULL(19,89)
  JVS(81) = B(79)
! JVS(82) = Jac_FULL(20,20)
  JVS(82) = -B(100)
! JVS(83) = Jac_FULL(20,84)
  JVS(83) = -B(101)
! JVS(84) = Jac_FULL(21,21)
  JVS(84) = -B(160)
! JVS(85) = Jac_FULL(21,52)
  JVS(85) = B(446)
! JVS(86) = Jac_FULL(21,84)
  JVS(86) = -B(161)
! JVS(87) = Jac_FULL(21,85)
  JVS(87) = B(447)
! JVS(88) = Jac_FULL(22,22)
  JVS(88) = -B(96)
! JVS(89) = Jac_FULL(22,84)
  JVS(89) = -B(97)
! JVS(90) = Jac_FULL(23,23)
  JVS(90) = -B(138)
! JVS(91) = Jac_FULL(23,71)
  JVS(91) = 0.35*B(136)
! JVS(92) = Jac_FULL(23,84)
  JVS(92) = 0.35*B(137)-B(139)
! JVS(93) = Jac_FULL(24,24)
  JVS(93) = -B(88)
! JVS(94) = Jac_FULL(24,41)
  JVS(94) = B(442)
! JVS(95) = Jac_FULL(24,84)
  JVS(95) = -B(89)+B(443)
! JVS(96) = Jac_FULL(25,25)
  JVS(96) = -B(94)
! JVS(97) = Jac_FULL(25,49)
  JVS(97) = 0.06*B(462)
! JVS(98) = Jac_FULL(25,73)
  JVS(98) = 0.06*B(186)
! JVS(99) = Jac_FULL(25,75)
  JVS(99) = 0.03*B(184)
! JVS(100) = Jac_FULL(25,79)
  JVS(100) = 0.03*B(185)+0.06*B(187)+0.06*B(463)
! JVS(101) = Jac_FULL(25,84)
  JVS(101) = -B(95)
! JVS(102) = Jac_FULL(26,26)
  JVS(102) = -B(92)
! JVS(103) = Jac_FULL(26,49)
  JVS(103) = 0.07*B(462)
! JVS(104) = Jac_FULL(26,73)
  JVS(104) = 0.07*B(186)
! JVS(105) = Jac_FULL(26,75)
  JVS(105) = 0.06*B(184)
! JVS(106) = Jac_FULL(26,79)
  JVS(106) = 0.06*B(185)+0.07*B(187)+0.07*B(463)
! JVS(107) = Jac_FULL(26,84)
  JVS(107) = -B(93)
! JVS(108) = Jac_FULL(27,27)
  JVS(108) = -B(13)-B(140)
! JVS(109) = Jac_FULL(27,83)
  JVS(109) = B(268)
! JVS(110) = Jac_FULL(27,84)
  JVS(110) = -B(141)
! JVS(111) = Jac_FULL(27,85)
  JVS(111) = B(269)
! JVS(112) = Jac_FULL(28,28)
  JVS(112) = -B(406)-B(451)
! JVS(113) = Jac_FULL(28,52)
  JVS(113) = B(449)
! JVS(114) = Jac_FULL(28,84)
  JVS(114) = -B(407)
! JVS(115) = Jac_FULL(28,88)
  JVS(115) = B(450)
! JVS(116) = Jac_FULL(29,29)
  JVS(116) = -B(6)-B(59)-B(66)
! JVS(117) = Jac_FULL(29,84)
  JVS(117) = -B(67)
! JVS(118) = Jac_FULL(29,85)
  JVS(118) = B(57)
! JVS(119) = Jac_FULL(29,88)
  JVS(119) = B(58)
! JVS(120) = Jac_FULL(30,28)
  JVS(120) = B(406)
! JVS(121) = Jac_FULL(30,30)
  JVS(121) = -B(90)
! JVS(122) = Jac_FULL(30,41)
  JVS(122) = B(442)
! JVS(123) = Jac_FULL(30,52)
  JVS(123) = 0.25*B(444)+B(448)
! JVS(124) = Jac_FULL(30,84)
  JVS(124) = -B(91)+B(407)+B(443)
! JVS(125) = Jac_FULL(30,87)
  JVS(125) = 0.25*B(445)
! JVS(126) = Jac_FULL(30,88)
  JVS(126) = 0
! JVS(127) = Jac_FULL(31,31)
  JVS(127) = -B(4)-B(62)
! JVS(128) = Jac_FULL(31,36)
  JVS(128) = B(206)
! JVS(129) = Jac_FULL(31,37)
  JVS(129) = B(212)
! JVS(130) = Jac_FULL(31,39)
  JVS(130) = B(218)
! JVS(131) = Jac_FULL(31,84)
  JVS(131) = B(49)-B(63)
! JVS(132) = Jac_FULL(31,87)
  JVS(132) = B(50)
! JVS(133) = Jac_FULL(31,88)
  JVS(133) = B(207)+B(213)+B(219)
! JVS(134) = Jac_FULL(32,20)
  JVS(134) = 0.024*B(100)
! JVS(135) = Jac_FULL(32,32)
  JVS(135) = -B(23)-B(128)
! JVS(136) = Jac_FULL(32,54)
  JVS(136) = 0.6*B(148)
! JVS(137) = Jac_FULL(32,81)
  JVS(137) = 0.3*B(345)
! JVS(138) = Jac_FULL(32,83)
  JVS(138) = 0.3*B(346)
! JVS(139) = Jac_FULL(32,84)
  JVS(139) = 0.024*B(101)-B(129)+0.6*B(149)
! JVS(140) = Jac_FULL(33,13)
  JVS(140) = B(28)
! JVS(141) = Jac_FULL(33,33)
  JVS(141) = -B(24)-B(26)-B(43)-B(45)-B(47)
! JVS(142) = Jac_FULL(33,51)
  JVS(142) = 0.09*B(188)
! JVS(143) = Jac_FULL(33,79)
  JVS(143) = B(3)-B(27)+0.09*B(189)
! JVS(144) = Jac_FULL(33,87)
  JVS(144) = -B(44)
! JVS(145) = Jac_FULL(33,88)
  JVS(145) = B(1)-B(46)-B(48)
! JVS(146) = Jac_FULL(33,89)
  JVS(146) = B(8)
! JVS(147) = Jac_FULL(34,34)
  JVS(147) = -B(202)-B(204)
! JVS(148) = Jac_FULL(34,50)
  JVS(148) = 0.1*B(120)+B(164)
! JVS(149) = Jac_FULL(34,84)
  JVS(149) = 0.1*B(121)
! JVS(150) = Jac_FULL(34,85)
  JVS(150) = -B(205)
! JVS(151) = Jac_FULL(34,88)
  JVS(151) = -B(203)
! JVS(152) = Jac_FULL(34,89)
  JVS(152) = B(165)
! JVS(153) = Jac_FULL(35,35)
  JVS(153) = -B(9)-B(38)
! JVS(154) = Jac_FULL(35,45)
  JVS(154) = 0.02*B(192)
! JVS(155) = Jac_FULL(35,47)
  JVS(155) = 0.02*B(194)
! JVS(156) = Jac_FULL(35,48)
  JVS(156) = 0.09*B(190)
! JVS(157) = Jac_FULL(35,49)
  JVS(157) = 0.011*B(462)
! JVS(158) = Jac_FULL(35,51)
  JVS(158) = 0.001*B(188)
! JVS(159) = Jac_FULL(35,73)
  JVS(159) = 0.011*B(186)
! JVS(160) = Jac_FULL(35,75)
  JVS(160) = 0.006*B(184)
! JVS(161) = Jac_FULL(35,79)
  JVS(161) = 0.006*B(185)+0.011*B(187)+0.001*B(189)+0.09*B(191)+0.02*B(193)+0.02*B(195)+0.011*B(463)
! JVS(162) = Jac_FULL(35,84)
  JVS(162) = -B(39)
! JVS(163) = Jac_FULL(35,85)
  JVS(163) = B(40)+B(41)
! JVS(164) = Jac_FULL(36,16)
  JVS(164) = 0.9*B(116)
! JVS(165) = Jac_FULL(36,36)
  JVS(165) = -B(206)-B(208)-B(210)
! JVS(166) = Jac_FULL(36,79)
  JVS(166) = -B(211)
! JVS(167) = Jac_FULL(36,84)
  JVS(167) = 0.9*B(117)
! JVS(168) = Jac_FULL(36,88)
  JVS(168) = -B(207)
! JVS(169) = Jac_FULL(37,17)
  JVS(169) = 0.9*B(118)
! JVS(170) = Jac_FULL(37,37)
  JVS(170) = -B(212)-B(214)-B(216)
! JVS(171) = Jac_FULL(37,79)
  JVS(171) = -B(217)
! JVS(172) = Jac_FULL(37,84)
  JVS(172) = 0.9*B(119)
! JVS(173) = Jac_FULL(37,88)
  JVS(173) = -B(213)
! JVS(174) = Jac_FULL(38,38)
  JVS(174) = -B(102)-B(166)-B(182)
! JVS(175) = Jac_FULL(38,79)
  JVS(175) = -B(183)
! JVS(176) = Jac_FULL(38,84)
  JVS(176) = -B(103)
! JVS(177) = Jac_FULL(38,89)
  JVS(177) = -B(167)
! JVS(178) = Jac_FULL(39,39)
  JVS(178) = -B(218)-B(220)-B(222)
! JVS(179) = Jac_FULL(39,50)
  JVS(179) = 0.85*B(120)
! JVS(180) = Jac_FULL(39,79)
  JVS(180) = -B(223)
! JVS(181) = Jac_FULL(39,84)
  JVS(181) = 0.85*B(121)
! JVS(182) = Jac_FULL(39,88)
  JVS(182) = -B(219)
! JVS(183) = Jac_FULL(40,40)
  JVS(183) = -B(15)-B(144)
! JVS(184) = Jac_FULL(40,71)
  JVS(184) = 0.11*B(198)
! JVS(185) = Jac_FULL(40,79)
  JVS(185) = 0.11*B(199)
! JVS(186) = Jac_FULL(40,82)
  JVS(186) = B(296)
! JVS(187) = Jac_FULL(40,84)
  JVS(187) = -B(145)
! JVS(188) = Jac_FULL(40,85)
  JVS(188) = B(297)
! JVS(189) = Jac_FULL(41,41)
  JVS(189) = -B(442)
! JVS(190) = Jac_FULL(41,48)
  JVS(190) = B(174)
! JVS(191) = Jac_FULL(41,60)
  JVS(191) = 0.046*B(246)
! JVS(192) = Jac_FULL(41,84)
  JVS(192) = -B(443)
! JVS(193) = Jac_FULL(41,87)
  JVS(193) = 0.046*B(247)
! JVS(194) = Jac_FULL(41,89)
  JVS(194) = B(175)
! JVS(195) = Jac_FULL(42,42)
  JVS(195) = -B(452)-B(454)-B(456)
! JVS(196) = Jac_FULL(42,79)
  JVS(196) = -B(455)
! JVS(197) = Jac_FULL(42,84)
  JVS(197) = -B(453)
! JVS(198) = Jac_FULL(42,89)
  JVS(198) = -B(457)
! JVS(199) = Jac_FULL(43,43)
  JVS(199) = -B(5)-B(64)
! JVS(200) = Jac_FULL(43,50)
  JVS(200) = B(164)
! JVS(201) = Jac_FULL(43,53)
  JVS(201) = B(156)
! JVS(202) = Jac_FULL(43,62)
  JVS(202) = B(158)
! JVS(203) = Jac_FULL(43,66)
  JVS(203) = B(152)
! JVS(204) = Jac_FULL(43,71)
  JVS(204) = 0.5*B(162)
! JVS(205) = Jac_FULL(43,74)
  JVS(205) = B(154)
! JVS(206) = Jac_FULL(43,84)
  JVS(206) = B(51)-B(65)
! JVS(207) = Jac_FULL(43,85)
  JVS(207) = 0.3*B(60)
! JVS(208) = Jac_FULL(43,88)
  JVS(208) = B(52)
! JVS(209) = Jac_FULL(43,89)
  JVS(209) = 0.3*B(61)+B(153)+B(155)+B(157)+B(159)+0.5*B(163)+B(165)
! JVS(210) = Jac_FULL(44,44)
  JVS(210) = -B(146)-B(226)
! JVS(211) = Jac_FULL(44,54)
  JVS(211) = 0.4*B(148)+0.4*B(180)+0.3*B(200)
! JVS(212) = Jac_FULL(44,79)
  JVS(212) = 0.3*B(201)
! JVS(213) = Jac_FULL(44,82)
  JVS(213) = B(224)
! JVS(214) = Jac_FULL(44,84)
  JVS(214) = -B(147)+0.4*B(149)
! JVS(215) = Jac_FULL(44,88)
  JVS(215) = B(225)
! JVS(216) = Jac_FULL(44,89)
  JVS(216) = 0.4*B(181)
! JVS(217) = Jac_FULL(45,45)
  JVS(217) = -B(112)-B(176)-B(192)
! JVS(218) = Jac_FULL(45,79)
  JVS(218) = -B(193)
! JVS(219) = Jac_FULL(45,84)
  JVS(219) = -B(113)
! JVS(220) = Jac_FULL(45,89)
  JVS(220) = -B(177)
! JVS(221) = Jac_FULL(46,22)
  JVS(221) = 0.036*B(96)
! JVS(222) = Jac_FULL(46,24)
  JVS(222) = B(88)
! JVS(223) = Jac_FULL(46,38)
  JVS(223) = 0.43*B(182)
! JVS(224) = Jac_FULL(46,41)
  JVS(224) = 0
! JVS(225) = Jac_FULL(46,45)
  JVS(225) = 0.14*B(192)
! JVS(226) = Jac_FULL(46,46)
  JVS(226) = -B(86)
! JVS(227) = Jac_FULL(46,47)
  JVS(227) = 0.14*B(194)
! JVS(228) = Jac_FULL(46,48)
  JVS(228) = 0.14*B(190)
! JVS(229) = Jac_FULL(46,49)
  JVS(229) = 0.3*B(462)
! JVS(230) = Jac_FULL(46,51)
  JVS(230) = 0.36*B(188)
! JVS(231) = Jac_FULL(46,52)
  JVS(231) = 0.25*B(444)+0.5*B(448)
! JVS(232) = Jac_FULL(46,53)
  JVS(232) = 1.87*B(17)+1.55*B(18)+2*B(130)+2*B(156)
! JVS(233) = Jac_FULL(46,54)
  JVS(233) = 0.13*B(200)
! JVS(234) = Jac_FULL(46,57)
  JVS(234) = B(22)+0.22*B(196)
! JVS(235) = Jac_FULL(46,60)
  JVS(235) = 0
! JVS(236) = Jac_FULL(46,62)
  JVS(236) = B(19)+B(132)+B(158)
! JVS(237) = Jac_FULL(46,66)
  JVS(237) = B(10)+B(11)+B(122)+B(152)
! JVS(238) = Jac_FULL(46,71)
  JVS(238) = 0.66*B(198)
! JVS(239) = Jac_FULL(46,73)
  JVS(239) = 0.3*B(186)
! JVS(240) = Jac_FULL(46,74)
  JVS(240) = B(12)
! JVS(241) = Jac_FULL(46,75)
  JVS(241) = 0.37*B(184)
! JVS(242) = Jac_FULL(46,79)
...
! JVS(243) = Jac_FULL(46,84)
  JVS(243) = -B(87)+B(89)+0.036*B(97)+B(123)+2*B(131)+B(133)
! JVS(244) = Jac_FULL(46,87)
  JVS(244) = 0.25*B(445)
! JVS(245) = Jac_FULL(46,89)
  JVS(245) = B(153)+2*B(157)+B(159)
! JVS(246) = Jac_FULL(47,47)
  JVS(246) = -B(114)-B(178)-B(194)
! JVS(247) = Jac_FULL(47,79)
  JVS(247) = -B(195)
! JVS(248) = Jac_FULL(47,84)
  JVS(248) = -B(115)
! JVS(249) = Jac_FULL(47,89)
  JVS(249) = -B(179)
! JVS(250) = Jac_FULL(48,48)
  JVS(250) = -B(110)-B(174)-B(190)
! JVS(251) = Jac_FULL(48,79)
  JVS(251) = -B(191)
! JVS(252) = Jac_FULL(48,84)
  JVS(252) = -B(111)
! JVS(253) = Jac_FULL(48,89)
  JVS(253) = -B(175)
! JVS(254) = Jac_FULL(49,49)
  JVS(254) = -B(458)-B(460)-B(462)
! JVS(255) = Jac_FULL(49,79)
  JVS(255) = -B(463)
! JVS(256) = Jac_FULL(49,84)
  JVS(256) = -B(459)
! JVS(257) = Jac_FULL(49,89)
  JVS(257) = -B(461)
! JVS(258) = Jac_FULL(50,34)
  JVS(258) = 0.1*B(202)+B(204)
! JVS(259) = Jac_FULL(50,36)
  JVS(259) = B(206)+0.02*B(208)+B(210)
! JVS(260) = Jac_FULL(50,37)
  JVS(260) = B(212)+0.02*B(214)+B(216)
! JVS(261) = Jac_FULL(50,39)
  JVS(261) = B(218)+0.02*B(220)+B(222)
! JVS(262) = Jac_FULL(50,50)
  JVS(262) = -B(120)-B(164)
! JVS(263) = Jac_FULL(50,79)
  JVS(263) = B(211)+B(217)+B(223)
! JVS(264) = Jac_FULL(50,84)
  JVS(264) = -B(121)
! JVS(265) = Jac_FULL(50,85)
  JVS(265) = B(205)
! JVS(266) = Jac_FULL(50,88)
  JVS(266) = 0.1*B(203)+B(207)+B(213)+B(219)
! JVS(267) = Jac_FULL(50,89)
  JVS(267) = -B(165)
! JVS(268) = Jac_FULL(51,51)
  JVS(268) = -B(108)-B(172)-B(188)
! JVS(269) = Jac_FULL(51,79)
  JVS(269) = -B(189)
! JVS(270) = Jac_FULL(51,84)
  JVS(270) = -B(109)
! JVS(271) = Jac_FULL(51,89)
  JVS(271) = -B(173)
! JVS(272) = Jac_FULL(52,21)
  JVS(272) = B(160)
! JVS(273) = Jac_FULL(52,28)
  JVS(273) = B(451)
! JVS(274) = Jac_FULL(52,48)
  JVS(274) = 0.1*B(190)
! JVS(275) = Jac_FULL(52,52)
  JVS(275) = -B(444)-B(446)-2*B(448)-B(449)
! JVS(276) = Jac_FULL(52,57)
  JVS(276) = B(134)
! JVS(277) = Jac_FULL(52,79)
  JVS(277) = 0.1*B(191)
! JVS(278) = Jac_FULL(52,84)
  JVS(278) = B(135)+B(161)
! JVS(279) = Jac_FULL(52,85)
  JVS(279) = -B(447)
! JVS(280) = Jac_FULL(52,87)
  JVS(280) = -B(445)
! JVS(281) = Jac_FULL(52,88)
  JVS(281) = -B(450)
! JVS(282) = Jac_FULL(52,89)
  JVS(282) = 0
! JVS(283) = Jac_FULL(53,22)
  JVS(283) = 0.036*B(96)
! JVS(284) = Jac_FULL(53,53)
  JVS(284) = -B(17)-B(18)-B(130)-B(156)
! JVS(285) = Jac_FULL(53,61)
  JVS(285) = B(256)+B(335)+B(375)+B(416)
! JVS(286) = Jac_FULL(53,67)
  JVS(286) = 1.2*B(252)+0.65*B(331)+0.65*B(371)+1.3*B(412)
! JVS(287) = Jac_FULL(53,68)
  JVS(287) = 0.35*B(254)+0.37*B(333)+0.37*B(373)+0.74*B(414)
! JVS(288) = Jac_FULL(53,71)
  JVS(288) = 0.15*B(136)+0.25*B(162)+0.5*B(198)
! JVS(289) = Jac_FULL(53,79)
  JVS(289) = 0.5*B(199)
! JVS(290) = Jac_FULL(53,80)
  JVS(290) = 0.063*B(234)+0.119*B(313)+0.1*B(353)+0.063*B(394)
! JVS(291) = Jac_FULL(53,82)
  JVS(291) = 0.1*B(354)+0.65*B(372)+0.37*B(374)+B(376)
! JVS(292) = Jac_FULL(53,83)
  JVS(292) = 0.119*B(314)+0.65*B(332)+0.37*B(334)+B(336)
! JVS(293) = Jac_FULL(53,84)
  JVS(293) = 0.036*B(97)-B(131)+0.15*B(137)
! JVS(294) = Jac_FULL(53,87)
  JVS(294) = 0.063*B(235)+1.2*B(253)+0.35*B(255)+B(257)
! JVS(295) = Jac_FULL(53,89)
  JVS(295) = -B(157)+0.25*B(163)+0.063*B(395)+1.3*B(413)+0.74*B(415)+B(417)
! JVS(296) = Jac_FULL(54,54)
  JVS(296) = -B(148)-B(180)-B(200)-B(229)
! JVS(297) = Jac_FULL(54,78)
  JVS(297) = B(227)
! JVS(298) = Jac_FULL(54,79)
  JVS(298) = -B(201)
! JVS(299) = Jac_FULL(54,84)
  JVS(299) = -B(149)
! JVS(300) = Jac_FULL(54,88)
  JVS(300) = B(228)
! JVS(301) = Jac_FULL(54,89)
  JVS(301) = -B(181)
! JVS(302) = Jac_FULL(55,38)
  JVS(302) = B(102)
! JVS(303) = Jac_FULL(55,55)
  JVS(303) = -B(240)-B(278)-B(319)-B(359)-B(400)
! JVS(304) = Jac_FULL(55,79)
  JVS(304) = 0
! JVS(305) = Jac_FULL(55,82)
  JVS(305) = -B(360)
! JVS(306) = Jac_FULL(55,83)
  JVS(306) = -B(320)
! JVS(307) = Jac_FULL(55,84)
  JVS(307) = B(103)
! JVS(308) = Jac_FULL(55,85)
  JVS(308) = -B(279)
! JVS(309) = Jac_FULL(55,87)
  JVS(309) = -B(241)
! JVS(310) = Jac_FULL(55,89)
  JVS(310) = -B(401)
! JVS(311) = Jac_FULL(56,56)
  JVS(311) = -B(242)-B(280)-B(321)-B(361)-B(402)
! JVS(312) = Jac_FULL(56,75)
  JVS(312) = B(104)
! JVS(313) = Jac_FULL(56,82)
  JVS(313) = -B(362)
! JVS(314) = Jac_FULL(56,83)
  JVS(314) = -B(322)
! JVS(315) = Jac_FULL(56,84)
  JVS(315) = B(105)
! JVS(316) = Jac_FULL(56,85)
  JVS(316) = -B(281)
! JVS(317) = Jac_FULL(56,87)
  JVS(317) = -B(243)
! JVS(318) = Jac_FULL(56,89)
  JVS(318) = -B(403)
! JVS(319) = Jac_FULL(57,15)
  JVS(319) = B(440)
! JVS(320) = Jac_FULL(57,42)
  JVS(320) = 0.9*B(456)
! JVS(321) = Jac_FULL(57,47)
  JVS(321) = 0.79*B(194)
! JVS(322) = Jac_FULL(57,48)
  JVS(322) = 0.65*B(190)
! JVS(323) = Jac_FULL(57,51)
  JVS(323) = 0.9*B(172)+0.39*B(188)
! JVS(324) = Jac_FULL(57,57)
  JVS(324) = -B(22)-B(134)-B(196)
! JVS(325) = Jac_FULL(57,60)
  JVS(325) = B(246)+0.55*B(325)+0.771*B(365)+2*B(439)
! JVS(326) = Jac_FULL(57,63)
  JVS(326) = 0.4*B(250)+0.6*B(329)+0.6*B(369)+0.6*B(410)
! JVS(327) = Jac_FULL(57,79)
  JVS(327) = 0.39*B(189)+0.65*B(191)+0.79*B(195)-B(197)
! JVS(328) = Jac_FULL(57,82)
  JVS(328) = 0.771*B(366)+0.6*B(370)
! JVS(329) = Jac_FULL(57,83)
  JVS(329) = 0.55*B(326)+0.6*B(330)
! JVS(330) = Jac_FULL(57,84)
  JVS(330) = -B(135)+B(441)
! JVS(331) = Jac_FULL(57,85)
  JVS(331) = 0
! JVS(332) = Jac_FULL(57,87)
  JVS(332) = B(247)+0.4*B(251)
! JVS(333) = Jac_FULL(57,89)
  JVS(333) = 0.9*B(173)+0.6*B(411)+0.9*B(457)
! JVS(334) = Jac_FULL(58,45)
  JVS(334) = B(112)
! JVS(335) = Jac_FULL(58,58)
  JVS(335) = -B(248)-B(286)-B(327)-B(367)-B(408)
! JVS(336) = Jac_FULL(58,79)
  JVS(336) = 0
! JVS(337) = Jac_FULL(58,82)
  JVS(337) = -B(368)
! JVS(338) = Jac_FULL(58,83)
  JVS(338) = -B(328)
! JVS(339) = Jac_FULL(58,84)
  JVS(339) = B(113)
! JVS(340) = Jac_FULL(58,85)
  JVS(340) = -B(287)
! JVS(341) = Jac_FULL(58,87)
  JVS(341) = -B(249)
! JVS(342) = Jac_FULL(58,89)
  JVS(342) = -B(409)
! JVS(343) = Jac_FULL(59,18)
  JVS(343) = 0.25*B(98)
! JVS(344) = Jac_FULL(59,23)
  JVS(344) = 0.12*B(138)
! JVS(345) = Jac_FULL(59,42)
  JVS(345) = 0.36*B(452)+0.23*B(454)
! JVS(346) = Jac_FULL(59,45)
  JVS(346) = 0.53*B(192)
! JVS(347) = Jac_FULL(59,49)
  JVS(347) = 0.16*B(462)
! JVS(348) = Jac_FULL(59,56)
  JVS(348) = 0.06*B(242)+0.081*B(321)+0.141*B(361)+0.06*B(402)
! JVS(349) = Jac_FULL(59,58)
  JVS(349) = 0.8*B(248)+B(327)+B(367)+B(408)
! JVS(350) = Jac_FULL(59,59)
  JVS(350) = -B(16)-B(126)
! JVS(351) = Jac_FULL(59,64)
  JVS(351) = 0.722*B(236)+0.24*B(315)+0.33*B(355)+0.828*B(396)
! JVS(352) = Jac_FULL(59,65)
  JVS(352) = 0.642*B(238)+0.419*B(317)+0.581*B(357)+0.88*B(398)
! JVS(353) = Jac_FULL(59,69)
  JVS(353) = 0.29*B(244)+0.313*B(323)+0.569*B(363)+0.29*B(404)
! JVS(354) = Jac_FULL(59,70)
  JVS(354) = 0.8*B(21)
! JVS(355) = Jac_FULL(59,71)
  JVS(355) = 0.03*B(162)
! JVS(356) = Jac_FULL(59,73)
  JVS(356) = 0.16*B(186)
! JVS(357) = Jac_FULL(59,75)
  JVS(357) = 0.03*B(184)
! JVS(358) = Jac_FULL(59,76)
  JVS(358) = 0.464*B(266)+0.149*B(349)+0.167*B(384)+0.149*B(387)+0.285*B(389)+0.469*B(426)
! JVS(359) = Jac_FULL(59,77)
  JVS(359) = 0.149*B(388)
! JVS(360) = Jac_FULL(59,79)
  JVS(360) = 0.03*B(185)+0.16*B(187)+0.53*B(193)+0.23*B(455)+0.16*B(463)
! JVS(361) = Jac_FULL(59,80)
  JVS(361) = 0.623*B(234)+0.018*B(313)+0.127*B(353)+0.67*B(394)
! JVS(362) = Jac_FULL(59,81)
  JVS(362) = 0.11*B(380)
! JVS(363) = Jac_FULL(59,82)
  JVS(363) = 0.127*B(354)+0.33*B(356)+0.581*B(358)+0.141*B(362)+0.569*B(364)+B(368)+0.11*B(381)+0.167*B(385)
! JVS(364) = Jac_FULL(59,83)
  JVS(364) = 0.018*B(314)+0.24*B(316)+0.419*B(318)+0.081*B(322)+0.313*B(324)+B(328)+0.149*B(350)
! JVS(365) = Jac_FULL(59,84)
  JVS(365) = 0.25*B(99)-B(127)+0.12*B(139)+0.41*B(142)+0.36*B(453)
! JVS(366) = Jac_FULL(59,85)
  JVS(366) = 0
! JVS(367) = Jac_FULL(59,86)
  JVS(367) = 0.41*B(143)
! JVS(368) = Jac_FULL(59,87)
  JVS(368) = 0.623*B(235)+0.722*B(237)+0.642*B(239)+0.06*B(243)+0.29*B(245)+0.8*B(249)+0.464*B(267)
! JVS(369) = Jac_FULL(59,89)
  JVS(369) = 0.03*B(163)+0.67*B(395)+0.828*B(397)+0.88*B(399)+0.06*B(403)+0.29*B(405)+B(409)+0.469*B(427)
! JVS(370) = Jac_FULL(60,48)
  JVS(370) = B(110)
! JVS(371) = Jac_FULL(60,51)
  JVS(371) = B(108)
! JVS(372) = Jac_FULL(60,60)
  JVS(372) = -B(246)-B(284)-B(325)-B(365)-2*B(439)
! JVS(373) = Jac_FULL(60,79)
  JVS(373) = 0
! JVS(374) = Jac_FULL(60,82)
  JVS(374) = -B(366)
! JVS(375) = Jac_FULL(60,83)
  JVS(375) = -B(326)
! JVS(376) = Jac_FULL(60,84)
  JVS(376) = B(109)+B(111)
! JVS(377) = Jac_FULL(60,85)
  JVS(377) = -B(285)
! JVS(378) = Jac_FULL(60,87)
  JVS(378) = -B(247)
! JVS(379) = Jac_FULL(60,89)
  JVS(379) = 0
! JVS(380) = Jac_FULL(61,39)
  JVS(380) = 0.98*B(220)
! JVS(381) = Jac_FULL(61,50)
  JVS(381) = 0
! JVS(382) = Jac_FULL(61,61)
  JVS(382) = -B(256)-B(294)-B(335)-B(375)-B(416)
! JVS(383) = Jac_FULL(61,79)
  JVS(383) = 0
! JVS(384) = Jac_FULL(61,82)
  JVS(384) = -B(376)
! JVS(385) = Jac_FULL(61,83)
  JVS(385) = -B(336)
! JVS(386) = Jac_FULL(61,84)
  JVS(386) = 0
! JVS(387) = Jac_FULL(61,85)
  JVS(387) = -B(295)
! JVS(388) = Jac_FULL(61,87)
  JVS(388) = -B(257)
! JVS(389) = Jac_FULL(61,88)
  JVS(389) = 0
! JVS(390) = Jac_FULL(61,89)
  JVS(390) = -B(417)
! JVS(391) = Jac_FULL(62,30)
  JVS(391) = B(90)
! JVS(392) = Jac_FULL(62,32)
  JVS(392) = B(128)
! JVS(393) = Jac_FULL(62,41)
  JVS(393) = 0
! JVS(394) = Jac_FULL(62,48)
  JVS(394) = 0
! JVS(395) = Jac_FULL(62,52)
  JVS(395) = 0.5*B(444)+B(448)
! JVS(396) = Jac_FULL(62,54)
  JVS(396) = 0
! JVS(397) = Jac_FULL(62,57)
  JVS(397) = 0.9*B(196)
! JVS(398) = Jac_FULL(62,60)
  JVS(398) = 0
...
! JVS(401) = Jac_FULL(62,63)
  JVS(401) = 0
! JVS(402) = Jac_FULL(62,67)
  JVS(402) = 0.65*B(252)+0.35*B(331)+0.35*B(371)+0.7*B(412)
! JVS(403) = Jac_FULL(62,68)
  JVS(403) = 0.6*B(254)+0.63*B(333)+0.63*B(373)+1.26*B(414)
! JVS(404) = Jac_FULL(62,71)
  JVS(404) = 0.15*B(136)+0.25*B(162)+0.62*B(198)
! JVS(405) = Jac_FULL(62,78)
  JVS(405) = 0
! JVS(406) = Jac_FULL(62,79)
  JVS(406) = 0.9*B(197)+0.62*B(199)
! JVS(407) = Jac_FULL(62,80)
  JVS(407) = 0.005*B(313)+0.004*B(353)
! JVS(408) = Jac_FULL(62,81)
  JVS(408) = 0.54*B(262)+0.4*B(345)+0.54*B(380)+0.54*B(422)
! JVS(409) = Jac_FULL(62,82)
  JVS(409) = 0.004*B(354)+0.35*B(372)+0.63*B(374)+B(376)+0.54*B(381)
! JVS(410) = Jac_FULL(62,83)
  JVS(410) = 0.005*B(314)+0.35*B(332)+0.63*B(334)+B(336)+0.4*B(346)
! JVS(411) = Jac_FULL(62,84)
  JVS(411) = B(91)+B(129)-B(133)+0.15*B(137)
! JVS(412) = Jac_FULL(62,85)
  JVS(412) = 0
! JVS(413) = Jac_FULL(62,87)
  JVS(413) = 0.65*B(253)+0.6*B(255)+B(257)+0.54*B(263)+0.5*B(445)
! JVS(414) = Jac_FULL(62,88)
  JVS(414) = 0
! JVS(415) = Jac_FULL(62,89)
  JVS(415) = -B(159)+0.25*B(163)+0.7*B(413)+1.26*B(415)+B(417)+0.54*B(423)
! JVS(416) = Jac_FULL(63,47)
  JVS(416) = B(114)
! JVS(417) = Jac_FULL(63,63)
  JVS(417) = -B(250)-B(288)-B(329)-B(369)-B(410)
! JVS(418) = Jac_FULL(63,79)
  JVS(418) = 0
! JVS(419) = Jac_FULL(63,82)
  JVS(419) = -B(370)
! JVS(420) = Jac_FULL(63,83)
  JVS(420) = -B(330)
! JVS(421) = Jac_FULL(63,84)
  JVS(421) = B(115)
! JVS(422) = Jac_FULL(63,85)
  JVS(422) = -B(289)
! JVS(423) = Jac_FULL(63,87)
  JVS(423) = -B(251)
! JVS(424) = Jac_FULL(63,89)
  JVS(424) = -B(411)
! JVS(425) = Jac_FULL(64,18)
  JVS(425) = 0.75*B(98)
! JVS(426) = Jac_FULL(64,64)
  JVS(426) = -B(236)-B(274)-B(315)-B(355)-B(396)
! JVS(427) = Jac_FULL(64,82)
  JVS(427) = -B(356)
! JVS(428) = Jac_FULL(64,83)
  JVS(428) = -B(316)
! JVS(429) = Jac_FULL(64,84)
  JVS(429) = 0.75*B(99)
! JVS(430) = Jac_FULL(64,85)
  JVS(430) = -B(275)
! JVS(431) = Jac_FULL(64,87)
  JVS(431) = -B(237)
! JVS(432) = Jac_FULL(64,89)
  JVS(432) = -B(397)
! JVS(433) = Jac_FULL(65,20)
  JVS(433) = 0.9511*B(100)
! JVS(434) = Jac_FULL(65,65)
  JVS(434) = -B(238)-B(276)-B(317)-B(357)-B(398)
! JVS(435) = Jac_FULL(65,82)
  JVS(435) = -B(358)
! JVS(436) = Jac_FULL(65,83)
  JVS(436) = -B(318)
! JVS(437) = Jac_FULL(65,84)
  JVS(437) = 0.9511*B(101)
! JVS(438) = Jac_FULL(65,85)
  JVS(438) = -B(277)
! JVS(439) = Jac_FULL(65,87)
  JVS(439) = -B(239)
! JVS(440) = Jac_FULL(65,89)
  JVS(440) = -B(399)
! JVS(441) = Jac_FULL(66,22)
  JVS(441) = 0.01*B(96)
! JVS(442) = Jac_FULL(66,24)
  JVS(442) = B(88)
! JVS(443) = Jac_FULL(66,27)
  JVS(443) = B(13)+0.35*B(140)
! JVS(444) = Jac_FULL(66,32)
  JVS(444) = B(23)
! JVS(445) = Jac_FULL(66,38)
  JVS(445) = B(182)
! JVS(446) = Jac_FULL(66,40)
  JVS(446) = 0.35*B(144)
! JVS(447) = Jac_FULL(66,41)
  JVS(447) = 0
! JVS(448) = Jac_FULL(66,42)
  JVS(448) = 0.3*B(452)+0.51*B(454)
! JVS(449) = Jac_FULL(66,44)
  JVS(449) = B(146)
! JVS(450) = Jac_FULL(66,47)
  JVS(450) = 0.04*B(194)
! JVS(451) = Jac_FULL(66,48)
  JVS(451) = 0.58*B(190)
! JVS(452) = Jac_FULL(66,49)
  JVS(452) = 0.02*B(462)
! JVS(453) = Jac_FULL(66,51)
  JVS(453) = 0.9*B(188)
! JVS(454) = Jac_FULL(66,52)
  JVS(454) = 0.75*B(444)+0.5*B(448)
! JVS(455) = Jac_FULL(66,53)
  JVS(455) = 0.13*B(17)+0.45*B(18)
! JVS(456) = Jac_FULL(66,54)
  JVS(456) = 0.4*B(148)+0.4*B(180)+0.7*B(200)
! JVS(457) = Jac_FULL(66,55)
  JVS(457) = 1.6*B(240)+1.55*B(319)+0.8*B(359)+1.6*B(400)
! JVS(458) = Jac_FULL(66,56)
  JVS(458) = B(242)+1.25*B(321)+0.501*B(361)+B(402)
! JVS(459) = Jac_FULL(66,57)
  JVS(459) = B(22)
! JVS(460) = Jac_FULL(66,58)
  JVS(460) = B(327)
! JVS(461) = Jac_FULL(66,60)
  JVS(461) = B(246)+1.09*B(325)+0.34*B(365)+B(439)
! JVS(462) = Jac_FULL(66,61)
  JVS(462) = B(335)
! JVS(463) = Jac_FULL(66,63)
  JVS(463) = 0.25*B(250)+1.4*B(329)+0.4*B(369)+0.4*B(410)
! JVS(464) = Jac_FULL(66,64)
  JVS(464) = 0.021*B(236)+0.829*B(315)+0.076*B(355)+0.021*B(396)
! JVS(465) = Jac_FULL(66,65)
  JVS(465) = 0.753*B(317)
! JVS(466) = Jac_FULL(66,66)
  JVS(466) = -B(10)-B(11)-B(122)-B(152)
! JVS(467) = Jac_FULL(66,67)
  JVS(467) = B(331)
! JVS(468) = Jac_FULL(66,68)
  JVS(468) = B(333)
! JVS(469) = Jac_FULL(66,69)
  JVS(469) = 0.755*B(323)
! JVS(470) = Jac_FULL(66,71)
  JVS(470) = 0
! JVS(471) = Jac_FULL(66,72)
  JVS(471) = B(430)
! JVS(472) = Jac_FULL(66,73)
  JVS(472) = 0.02*B(186)
! JVS(473) = Jac_FULL(66,75)
  JVS(473) = 0.64*B(184)
! JVS(474) = Jac_FULL(66,76)
  JVS(474) = 0.287*B(266)+0.96*B(349)+0.207*B(384)+0.202*B(387)+0.504*B(389)+0.28*B(426)
! JVS(475) = Jac_FULL(66,77)
  JVS(475) = 0.75*B(347)+0.202*B(388)
! JVS(476) = Jac_FULL(66,78)
  JVS(476) = B(260)+2*B(341)+B(343)+B(378)+B(420)
! JVS(477) = Jac_FULL(66,79)
  JVS(477) = B(183)+0.64*B(185)+0.02*B(187)+0.9*B(189)+0.58*B(191)+0.04*B(195)+0.7*B(201)+0.51*B(455)+0.02*B(463)
! JVS(478) = Jac_FULL(66,80)
  JVS(478) = 0.047*B(234)+0.81*B(313)+0.091*B(353)+0.048*B(394)
! JVS(479) = Jac_FULL(66,81)
  JVS(479) = 0.75*B(345)
! JVS(480) = Jac_FULL(66,82)
  JVS(480) = B(337)+B(339)+0.091*B(354)+0.076*B(356)+0.8*B(360)+0.501*B(362)+0.34*B(366)+0.4*B(370)+B(379)+0.207*B(385)
! JVS(481) = Jac_FULL(66,83)
...
! JVS(482) = Jac_FULL(66,84)
  JVS(482) = B(89)+0.01*B(97)-B(123)+0.35*B(141)+0.35*B(145)+B(147)+0.4*B(149)+0.3*B(453)
! JVS(483) = Jac_FULL(66,85)
  JVS(483) = 0
! JVS(484) = Jac_FULL(66,87)
  JVS(484) = B(231)+0.047*B(235)+0.021*B(237)+1.6*B(241)+B(243)+B(247)+0.25*B(251)+B(261)+0.287*B(267)+0.75*B(445)
! JVS(485) = Jac_FULL(66,88)
  JVS(485) = 0
! JVS(486) = Jac_FULL(66,89)
  JVS(486) = -B(153)+0.4*B(181)+B(391)+0.048*B(395)+0.021*B(397)+1.6*B(401)+B(403)+0.4*B(411)+B(421)+0.28*B(427)
! JVS(487) = Jac_FULL(66,90)
  JVS(487) = 0.75*B(312)
! JVS(488) = Jac_FULL(67,36)
  JVS(488) = 0.98*B(208)
! JVS(489) = Jac_FULL(67,67)
  JVS(489) = -B(252)-B(290)-B(331)-B(371)-B(412)
! JVS(490) = Jac_FULL(67,79)
  JVS(490) = 0
! JVS(491) = Jac_FULL(67,82)
  JVS(491) = -B(372)
! JVS(492) = Jac_FULL(67,83)
  JVS(492) = -B(332)
! JVS(493) = Jac_FULL(67,84)
  JVS(493) = 0
! JVS(494) = Jac_FULL(67,85)
  JVS(494) = -B(291)
! JVS(495) = Jac_FULL(67,87)
  JVS(495) = -B(253)
! JVS(496) = Jac_FULL(67,88)
  JVS(496) = 0
! JVS(497) = Jac_FULL(67,89)
  JVS(497) = -B(413)
! JVS(498) = Jac_FULL(68,37)
  JVS(498) = 0.98*B(214)
! JVS(499) = Jac_FULL(68,68)
  JVS(499) = -B(254)-B(292)-B(333)-B(373)-B(414)
! JVS(500) = Jac_FULL(68,79)
  JVS(500) = 0
! JVS(501) = Jac_FULL(68,82)
  JVS(501) = -B(374)
! JVS(502) = Jac_FULL(68,83)
  JVS(502) = -B(334)
! JVS(503) = Jac_FULL(68,84)
  JVS(503) = 0
! JVS(504) = Jac_FULL(68,85)
  JVS(504) = -B(293)
! JVS(505) = Jac_FULL(68,87)
  JVS(505) = -B(255)
! JVS(506) = Jac_FULL(68,88)
  JVS(506) = 0
! JVS(507) = Jac_FULL(68,89)
  JVS(507) = -B(415)
! JVS(508) = Jac_FULL(69,42)
  JVS(508) = 0.19*B(452)
! JVS(509) = Jac_FULL(69,49)
  JVS(509) = B(458)
! JVS(510) = Jac_FULL(69,69)
  JVS(510) = -B(244)-B(282)-B(323)-B(363)-B(404)
! JVS(511) = Jac_FULL(69,73)
  JVS(511) = B(106)
! JVS(512) = Jac_FULL(69,79)
  JVS(512) = 0
! JVS(513) = Jac_FULL(69,82)
  JVS(513) = -B(364)
! JVS(514) = Jac_FULL(69,83)
  JVS(514) = -B(324)
! JVS(515) = Jac_FULL(69,84)
  JVS(515) = B(107)+0.19*B(453)+B(459)
! JVS(516) = Jac_FULL(69,85)
  JVS(516) = -B(283)
! JVS(517) = Jac_FULL(69,87)
  JVS(517) = -B(245)
! JVS(518) = Jac_FULL(69,89)
  JVS(518) = -B(405)
! JVS(519) = Jac_FULL(70,34)
  JVS(519) = B(202)
! JVS(520) = Jac_FULL(70,50)
  JVS(520) = 0
! JVS(521) = Jac_FULL(70,54)
  JVS(521) = 0.6*B(180)
! JVS(522) = Jac_FULL(70,58)
  JVS(522) = 0.2*B(248)
! JVS(523) = Jac_FULL(70,63)
  JVS(523) = 0.35*B(250)
! JVS(524) = Jac_FULL(70,64)
  JVS(524) = 0.124*B(236)
! JVS(525) = Jac_FULL(70,65)
  JVS(525) = 0.261*B(238)
! JVS(526) = Jac_FULL(70,67)
  JVS(526) = 0.05*B(252)
! JVS(527) = Jac_FULL(70,68)
  JVS(527) = 0.05*B(254)
! JVS(528) = Jac_FULL(70,70)
  JVS(528) = -B(21)-B(150)
! JVS(529) = Jac_FULL(70,76)
  JVS(529) = B(308)+0.5*B(349)+0.484*B(384)+1.5*B(387)+B(389)
! JVS(530) = Jac_FULL(70,77)
  JVS(530) = B(264)+B(306)+B(347)+B(382)+2*B(386)+1.5*B(388)+B(424)
! JVS(531) = Jac_FULL(70,78)
  JVS(531) = 0
! JVS(532) = Jac_FULL(70,79)
  JVS(532) = 0
! JVS(533) = Jac_FULL(70,80)
  JVS(533) = 0.059*B(234)
! JVS(534) = Jac_FULL(70,82)
  JVS(534) = B(383)+0.484*B(385)
! JVS(535) = Jac_FULL(70,83)
  JVS(535) = B(348)+0.5*B(350)
! JVS(536) = Jac_FULL(70,84)
  JVS(536) = -B(151)
! JVS(537) = Jac_FULL(70,85)
  JVS(537) = B(307)+B(309)
! JVS(538) = Jac_FULL(70,87)
  JVS(538) = 0.059*B(235)+0.124*B(237)+0.261*B(239)+0.2*B(249)+0.35*B(251)+0.05*B(253)+0.05*B(255)+B(265)
! JVS(539) = Jac_FULL(70,88)
  JVS(539) = B(203)
! JVS(540) = Jac_FULL(70,89)
  JVS(540) = 0.6*B(181)+B(425)
! JVS(541) = Jac_FULL(71,67)
  JVS(541) = 0.5*B(252)+B(331)+B(371)+0.5*B(412)
! JVS(542) = Jac_FULL(71,68)
  JVS(542) = 0.95*B(254)+B(333)+B(373)+B(414)
! JVS(543) = Jac_FULL(71,71)
  JVS(543) = -B(20)-B(136)-B(162)-B(198)
! JVS(544) = Jac_FULL(71,79)
  JVS(544) = -B(199)
! JVS(545) = Jac_FULL(71,82)
  JVS(545) = B(372)+B(374)
! JVS(546) = Jac_FULL(71,83)
  JVS(546) = B(332)+B(334)
! JVS(547) = Jac_FULL(71,84)
  JVS(547) = -B(137)
! JVS(548) = Jac_FULL(71,85)
  JVS(548) = 0
! JVS(549) = Jac_FULL(71,87)
  JVS(549) = 0.5*B(253)+0.95*B(255)
! JVS(550) = Jac_FULL(71,88)
  JVS(550) = 0
! JVS(551) = Jac_FULL(71,89)
  JVS(551) = -B(163)+0.5*B(413)+B(415)
! JVS(552) = Jac_FULL(72,16)
  JVS(552) = 0.1*B(116)
! JVS(553) = Jac_FULL(72,17)
  JVS(553) = 0.1*B(118)
! JVS(554) = Jac_FULL(72,40)
  JVS(554) = 0.35*B(144)
! JVS(555) = Jac_FULL(72,44)
  JVS(555) = B(146)
! JVS(556) = Jac_FULL(72,50)
  JVS(556) = 0.05*B(120)
! JVS(557) = Jac_FULL(72,51)
  JVS(557) = 0.13*B(188)
! JVS(558) = Jac_FULL(72,54)
  JVS(558) = B(148)+B(180)
! JVS(559) = Jac_FULL(72,64)
  JVS(559) = 0.334*B(236)+0.245*B(315)+0.237*B(355)+0.391*B(396)
! JVS(560) = Jac_FULL(72,65)
  JVS(560) = 0.416*B(238)+0.322*B(317)+0.318*B(357)+0.587*B(398)
! JVS(561) = Jac_FULL(72,71)
  JVS(561) = 0.5*B(136)+0.5*B(162)
! JVS(562) = Jac_FULL(72,72)
  JVS(562) = -B(428)-B(430)-B(432)-2*B(434)-B(435)-B(437)
! JVS(563) = Jac_FULL(72,78)
  JVS(563) = 0
! JVS(564) = Jac_FULL(72,79)
  JVS(564) = 0.13*B(189)
! JVS(565) = Jac_FULL(72,80)
  JVS(565) = 0.048*B(234)+0.085*B(313)+0.071*B(353)+0.051*B(394)
! JVS(566) = Jac_FULL(72,81)
  JVS(566) = 0.16*B(262)+0.08*B(345)+0.08*B(380)+0.16*B(422)
! JVS(567) = Jac_FULL(72,82)
  JVS(567) = 0.071*B(354)+0.237*B(356)+0.318*B(358)+0.08*B(381)-B(433)
! JVS(568) = Jac_FULL(72,83)
  JVS(568) = 0.085*B(314)+0.245*B(316)+0.322*B(318)+0.08*B(346)-B(431)
! JVS(569) = Jac_FULL(72,84)
  JVS(569) = 0.1*B(117)+0.1*B(119)+0.05*B(121)+0.5*B(137)+0.07*B(142)+0.35*B(145)+B(147)+B(149)
! JVS(570) = Jac_FULL(72,85)
  JVS(570) = -B(429)
! JVS(571) = Jac_FULL(72,86)
  JVS(571) = 0.07*B(143)
! JVS(572) = Jac_FULL(72,87)
  JVS(572) = 0.048*B(235)+0.334*B(237)+0.416*B(239)+0.16*B(263)-B(436)
! JVS(573) = Jac_FULL(72,88)
  JVS(573) = 0
! JVS(574) = Jac_FULL(72,89)
  JVS(574) = 0.5*B(163)+B(181)+0.051*B(395)+0.391*B(397)+0.587*B(399)+0.16*B(423)-B(438)
! JVS(575) = Jac_FULL(73,60)
  JVS(575) = 0.08*B(325)
! JVS(576) = Jac_FULL(73,63)
  JVS(576) = 0.25*B(250)+0.4*B(329)+0.4*B(369)+0.4*B(410)
! JVS(577) = Jac_FULL(73,73)
  JVS(577) = -B(106)-B(170)-B(186)
! JVS(578) = Jac_FULL(73,79)
  JVS(578) = -B(187)
! JVS(579) = Jac_FULL(73,82)
  JVS(579) = 0.4*B(370)
! JVS(580) = Jac_FULL(73,83)
  JVS(580) = 0.08*B(326)+0.4*B(330)
! JVS(581) = Jac_FULL(73,84)
  JVS(581) = -B(107)
! JVS(582) = Jac_FULL(73,85)
  JVS(582) = 0
! JVS(583) = Jac_FULL(73,87)
  JVS(583) = 0.25*B(251)
! JVS(584) = Jac_FULL(73,89)
  JVS(584) = -B(171)+0.4*B(411)
! JVS(585) = Jac_FULL(74,20)
  JVS(585) = 0.025*B(100)
! JVS(586) = Jac_FULL(74,22)
  JVS(586) = 0.335*B(96)
! JVS(587) = Jac_FULL(74,23)
  JVS(587) = 0.88*B(138)
! JVS(588) = Jac_FULL(74,42)
  JVS(588) = 0.85*B(454)
! JVS(589) = Jac_FULL(74,45)
  JVS(589) = 0.65*B(192)
! JVS(590) = Jac_FULL(74,49)
  JVS(590) = 0.99*B(462)
! JVS(591) = Jac_FULL(74,55)
  JVS(591) = 0.2*B(240)+0.35*B(319)+0.6*B(359)+0.2*B(400)
! JVS(592) = Jac_FULL(74,56)
  JVS(592) = 0.94*B(242)+0.669*B(321)+0.859*B(361)+0.94*B(402)
! JVS(593) = Jac_FULL(74,58)
  JVS(593) = 0.8*B(248)+B(327)+B(367)+B(408)
! JVS(594) = Jac_FULL(74,64)
  JVS(594) = 0.211*B(236)+0.523*B(315)+0.677*B(355)+0.239*B(396)
! JVS(595) = Jac_FULL(74,65)
  JVS(595) = 0.15*B(238)+0.411*B(317)+0.497*B(357)+0.187*B(398)
! JVS(596) = Jac_FULL(74,69)
  JVS(596) = 1.71*B(244)+0.932*B(323)+0.941*B(363)+1.71*B(404)
! JVS(597) = Jac_FULL(74,70)
  JVS(597) = 0.2*B(21)
! JVS(598) = Jac_FULL(74,71)
  JVS(598) = 0.25*B(162)+0.16*B(198)
! JVS(599) = Jac_FULL(74,73)
  JVS(599) = 0.99*B(186)
! JVS(600) = Jac_FULL(74,74)
  JVS(600) = -B(12)-B(124)-B(154)
! JVS(601) = Jac_FULL(74,75)
  JVS(601) = 0.44*B(184)
! JVS(602) = Jac_FULL(74,76)
  JVS(602) = 1.24*B(266)+0.64*B(349)+0.65*B(384)+0.64*B(387)+1.21*B(389)+1.24*B(426)
! JVS(603) = Jac_FULL(74,77)
  JVS(603) = 0.64*B(388)
! JVS(604) = Jac_FULL(74,78)
  JVS(604) = 0
! JVS(605) = Jac_FULL(74,79)
  JVS(605) = 0.44*B(185)+0.99*B(187)+0.65*B(193)+0.16*B(199)+0.85*B(455)+0.99*B(463)
! JVS(606) = Jac_FULL(74,80)
  JVS(606) = 0.233*B(234)+0.58*B(313)+0.724*B(353)+0.243*B(394)
! JVS(607) = Jac_FULL(74,81)
  JVS(607) = 0.46*B(262)+0.3*B(345)+0.35*B(380)+0.46*B(422)
! JVS(608) = Jac_FULL(74,82)
  JVS(608) = B(351)+0.724*B(354)+0.677*B(356)+0.497*B(358)+0.6*B(360)+0.859*B(362)+0.941*B(364)+B(368)+0.35*B(381)+0.65&
               &*B(385)
! JVS(609) = Jac_FULL(74,83)
  JVS(609) = 0.75*B(311)+0.58*B(314)+0.523*B(316)+0.411*B(318)+0.35*B(320)+0.669*B(322)+0.932*B(324)+B(328)+0.3*B(346)&
               &+0.64*B(350)
! JVS(610) = Jac_FULL(74,84)
  JVS(610) = 0.335*B(97)+0.025*B(101)-B(125)+0.88*B(139)+0.08*B(142)
! JVS(611) = Jac_FULL(74,85)
  JVS(611) = 0
! JVS(612) = Jac_FULL(74,86)
  JVS(612) = B(14)+0.08*B(143)
! JVS(613) = Jac_FULL(74,87)
  JVS(613) = B(232)+0.233*B(235)+0.211*B(237)+0.15*B(239)+0.2*B(241)+0.94*B(243)+1.71*B(245)+0.8*B(249)+0.46*B(263)+1.24&
               &*B(267)
! JVS(614) = Jac_FULL(74,88)
  JVS(614) = 0
! JVS(615) = Jac_FULL(74,89)
...
! JVS(616) = Jac_FULL(74,90)
  JVS(616) = B(233)+0.75*B(312)+B(352)+B(393)
! JVS(617) = Jac_FULL(75,47)
  JVS(617) = 0.46*B(194)
! JVS(618) = Jac_FULL(75,51)
  JVS(618) = 0.35*B(188)
! JVS(619) = Jac_FULL(75,60)
  JVS(619) = 0.37*B(325)+0.229*B(365)
! JVS(620) = Jac_FULL(75,75)
  JVS(620) = -B(104)-B(168)-B(184)
! JVS(621) = Jac_FULL(75,79)
  JVS(621) = -B(185)+0.35*B(189)+0.46*B(195)
! JVS(622) = Jac_FULL(75,82)
  JVS(622) = 0.229*B(366)
! JVS(623) = Jac_FULL(75,83)
  JVS(623) = 0.37*B(326)
! JVS(624) = Jac_FULL(75,84)
  JVS(624) = -B(105)
! JVS(625) = Jac_FULL(75,85)
  JVS(625) = 0
! JVS(626) = Jac_FULL(75,87)
  JVS(626) = 0
! JVS(627) = Jac_FULL(75,89)
  JVS(627) = -B(169)
! JVS(628) = Jac_FULL(76,38)
  JVS(628) = 0.2*B(166)
! JVS(629) = Jac_FULL(76,42)
  JVS(629) = 0.1*B(456)
! JVS(630) = Jac_FULL(76,45)
  JVS(630) = 0.9*B(176)
! JVS(631) = Jac_FULL(76,47)
  JVS(631) = 0.87*B(178)
! JVS(632) = Jac_FULL(76,49)
  JVS(632) = 0.89*B(460)
! JVS(633) = Jac_FULL(76,51)
  JVS(633) = 0.1*B(172)
! JVS(634) = Jac_FULL(76,73)
  JVS(634) = 0.89*B(170)
! JVS(635) = Jac_FULL(76,75)
  JVS(635) = 0.57*B(168)
! JVS(636) = Jac_FULL(76,76)
  JVS(636) = -B(266)-B(308)-B(349)-B(384)-B(387)-2*B(389)-B(426)
! JVS(637) = Jac_FULL(76,77)
  JVS(637) = -B(388)
! JVS(638) = Jac_FULL(76,79)
  JVS(638) = 0
! JVS(639) = Jac_FULL(76,82)
  JVS(639) = -B(385)
! JVS(640) = Jac_FULL(76,83)
  JVS(640) = -B(350)
! JVS(641) = Jac_FULL(76,84)
  JVS(641) = 0
! JVS(642) = Jac_FULL(76,85)
  JVS(642) = -B(309)
! JVS(643) = Jac_FULL(76,87)
  JVS(643) = -B(267)
! JVS(644) = Jac_FULL(76,89)
  JVS(644) = 0.2*B(167)+0.57*B(169)+0.89*B(171)+0.1*B(173)+0.9*B(177)+0.87*B(179)-B(427)+0.1*B(457)+0.89*B(461)
! JVS(645) = Jac_FULL(77,38)
  JVS(645) = 0.8*B(166)
! JVS(646) = Jac_FULL(77,42)
  JVS(646) = 0.9*B(456)
! JVS(647) = Jac_FULL(77,45)
  JVS(647) = 0.1*B(176)
! JVS(648) = Jac_FULL(77,47)
  JVS(648) = 0.13*B(178)
! JVS(649) = Jac_FULL(77,49)
  JVS(649) = 0.11*B(460)
! JVS(650) = Jac_FULL(77,51)
  JVS(650) = 0.9*B(172)
! JVS(651) = Jac_FULL(77,73)
  JVS(651) = 0.11*B(170)
! JVS(652) = Jac_FULL(77,75)
  JVS(652) = 0.43*B(168)
! JVS(653) = Jac_FULL(77,76)
  JVS(653) = -B(387)
! JVS(654) = Jac_FULL(77,77)
  JVS(654) = -B(264)-B(306)-B(347)-B(382)-2*B(386)-B(388)-B(424)
! JVS(655) = Jac_FULL(77,79)
  JVS(655) = 0
! JVS(656) = Jac_FULL(77,82)
  JVS(656) = -B(383)
! JVS(657) = Jac_FULL(77,83)
  JVS(657) = -B(348)
! JVS(658) = Jac_FULL(77,84)
  JVS(658) = 0
! JVS(659) = Jac_FULL(77,85)
  JVS(659) = -B(307)
! JVS(660) = Jac_FULL(77,87)
  JVS(660) = -B(265)
! JVS(661) = Jac_FULL(77,89)
  JVS(661) = 0.8*B(167)+0.43*B(169)+0.11*B(171)+0.9*B(173)+0.1*B(177)+0.13*B(179)-B(425)+0.9*B(457)+0.11*B(461)
! JVS(662) = Jac_FULL(78,54)
  JVS(662) = B(229)
! JVS(663) = Jac_FULL(78,71)
  JVS(663) = B(20)+0.5*B(136)+0.5*B(162)
! JVS(664) = Jac_FULL(78,78)
  JVS(664) = -B(227)-B(260)-B(300)-B(302)-B(341)-B(343)-B(378)-B(420)
! JVS(665) = Jac_FULL(78,79)
  JVS(665) = 0
! JVS(666) = Jac_FULL(78,82)
  JVS(666) = -B(379)
! JVS(667) = Jac_FULL(78,83)
  JVS(667) = -B(342)-B(344)
! JVS(668) = Jac_FULL(78,84)
  JVS(668) = 0.5*B(137)
! JVS(669) = Jac_FULL(78,85)
  JVS(669) = -B(301)-B(303)
! JVS(670) = Jac_FULL(78,87)
  JVS(670) = -B(261)
! JVS(671) = Jac_FULL(78,88)
  JVS(671) = -B(228)
! JVS(672) = Jac_FULL(78,89)
  JVS(672) = 0.5*B(163)-B(421)
! JVS(673) = Jac_FULL(79,33)
  JVS(673) = B(24)-B(26)
! JVS(674) = Jac_FULL(79,36)
  JVS(674) = -B(210)
! JVS(675) = Jac_FULL(79,37)
  JVS(675) = -B(216)
! JVS(676) = Jac_FULL(79,38)
  JVS(676) = -B(182)
! JVS(677) = Jac_FULL(79,39)
  JVS(677) = -B(222)
! JVS(678) = Jac_FULL(79,42)
  JVS(678) = -B(454)
! JVS(679) = Jac_FULL(79,45)
  JVS(679) = -B(192)
! JVS(680) = Jac_FULL(79,47)
  JVS(680) = -B(194)
! JVS(681) = Jac_FULL(79,48)
  JVS(681) = -B(190)
! JVS(682) = Jac_FULL(79,49)
  JVS(682) = -B(462)
! JVS(683) = Jac_FULL(79,50)
  JVS(683) = 0
! JVS(684) = Jac_FULL(79,51)
  JVS(684) = -B(188)
! JVS(685) = Jac_FULL(79,54)
  JVS(685) = -B(200)
! JVS(686) = Jac_FULL(79,57)
  JVS(686) = -B(196)
! JVS(687) = Jac_FULL(79,60)
  JVS(687) = 0
! JVS(688) = Jac_FULL(79,63)
  JVS(688) = 0
! JVS(689) = Jac_FULL(79,71)
  JVS(689) = -B(198)
! JVS(690) = Jac_FULL(79,73)
  JVS(690) = -B(186)
! JVS(691) = Jac_FULL(79,75)
  JVS(691) = -B(184)
! JVS(692) = Jac_FULL(79,78)
  JVS(692) = B(302)
! JVS(693) = Jac_FULL(79,79)
...
! JVS(694) = Jac_FULL(79,82)
  JVS(694) = B(298)
! JVS(695) = Jac_FULL(79,83)
  JVS(695) = 0
! JVS(696) = Jac_FULL(79,84)
  JVS(696) = -B(33)
! JVS(697) = Jac_FULL(79,85)
  JVS(697) = -B(35)+B(299)+B(303)
! JVS(698) = Jac_FULL(79,87)
  JVS(698) = -B(69)
! JVS(699) = Jac_FULL(79,88)
  JVS(699) = -B(71)
! JVS(700) = Jac_FULL(79,89)
  JVS(700) = 0
! JVS(701) = Jac_FULL(80,22)
  JVS(701) = 0.583*B(96)
! JVS(702) = Jac_FULL(80,70)
  JVS(702) = B(150)
! JVS(703) = Jac_FULL(80,76)
  JVS(703) = 0
! JVS(704) = Jac_FULL(80,77)
  JVS(704) = 0
! JVS(705) = Jac_FULL(80,78)
  JVS(705) = 0
! JVS(706) = Jac_FULL(80,79)
  JVS(706) = 0
! JVS(707) = Jac_FULL(80,80)
  JVS(707) = -B(234)-B(272)-B(313)-B(353)-B(394)
! JVS(708) = Jac_FULL(80,82)
  JVS(708) = -B(354)
! JVS(709) = Jac_FULL(80,83)
  JVS(709) = -B(314)
! JVS(710) = Jac_FULL(80,84)
  JVS(710) = 0.583*B(97)+0.44*B(142)+B(151)
! JVS(711) = Jac_FULL(80,85)
  JVS(711) = -B(273)
! JVS(712) = Jac_FULL(80,86)
  JVS(712) = 0.44*B(143)
! JVS(713) = Jac_FULL(80,87)
  JVS(713) = -B(235)
! JVS(714) = Jac_FULL(80,88)
  JVS(714) = 0
! JVS(715) = Jac_FULL(80,89)
  JVS(715) = -B(395)
! JVS(716) = Jac_FULL(81,45)
  JVS(716) = 0.42*B(192)
! JVS(717) = Jac_FULL(81,47)
  JVS(717) = 0.42*B(194)
! JVS(718) = Jac_FULL(81,49)
  JVS(718) = 0.12*B(462)
! JVS(719) = Jac_FULL(81,51)
  JVS(719) = 0.02*B(188)
! JVS(720) = Jac_FULL(81,59)
  JVS(720) = B(126)
! JVS(721) = Jac_FULL(81,64)
  JVS(721) = 0
! JVS(722) = Jac_FULL(81,65)
  JVS(722) = 0
! JVS(723) = Jac_FULL(81,69)
  JVS(723) = 0
! JVS(724) = Jac_FULL(81,70)
  JVS(724) = 0
! JVS(725) = Jac_FULL(81,71)
  JVS(725) = 0
! JVS(726) = Jac_FULL(81,73)
  JVS(726) = 0.12*B(186)
! JVS(727) = Jac_FULL(81,75)
  JVS(727) = 0.03*B(184)
! JVS(728) = Jac_FULL(81,76)
  JVS(728) = 0
! JVS(729) = Jac_FULL(81,77)
  JVS(729) = 0
! JVS(730) = Jac_FULL(81,78)
  JVS(730) = 0
! JVS(731) = Jac_FULL(81,79)
  JVS(731) = 0.03*B(185)+0.12*B(187)+0.02*B(189)+0.42*B(193)+0.42*B(195)+0.12*B(463)
! JVS(732) = Jac_FULL(81,80)
  JVS(732) = 0
! JVS(733) = Jac_FULL(81,81)
  JVS(733) = -B(262)-B(304)-B(345)-B(380)-B(422)
! JVS(734) = Jac_FULL(81,82)
  JVS(734) = -B(381)
! JVS(735) = Jac_FULL(81,83)
  JVS(735) = -B(346)
! JVS(736) = Jac_FULL(81,84)
  JVS(736) = B(127)
! JVS(737) = Jac_FULL(81,85)
  JVS(737) = -B(305)
! JVS(738) = Jac_FULL(81,86)
  JVS(738) = 0
! JVS(739) = Jac_FULL(81,87)
  JVS(739) = -B(263)
! JVS(740) = Jac_FULL(81,88)
  JVS(740) = 0
! JVS(741) = Jac_FULL(81,89)
  JVS(741) = -B(423)
! JVS(742) = Jac_FULL(82,32)
  JVS(742) = B(23)
! JVS(743) = Jac_FULL(82,40)
  JVS(743) = 0.65*B(144)
! JVS(744) = Jac_FULL(82,44)
  JVS(744) = B(226)
! JVS(745) = Jac_FULL(82,48)
  JVS(745) = 0.1*B(190)
! JVS(746) = Jac_FULL(82,51)
  JVS(746) = 0.15*B(188)
! JVS(747) = Jac_FULL(82,52)
  JVS(747) = 0.25*B(444)
! JVS(748) = Jac_FULL(82,54)
  JVS(748) = 0.7*B(200)
! JVS(749) = Jac_FULL(82,55)
  JVS(749) = -B(359)
! JVS(750) = Jac_FULL(82,56)
  JVS(750) = -B(361)
! JVS(751) = Jac_FULL(82,57)
  JVS(751) = B(22)+0.1*B(196)
! JVS(752) = Jac_FULL(82,58)
  JVS(752) = -B(367)
! JVS(753) = Jac_FULL(82,59)
  JVS(753) = B(16)
! JVS(754) = Jac_FULL(82,60)
  JVS(754) = -B(365)
! JVS(755) = Jac_FULL(82,61)
  JVS(755) = -B(375)
! JVS(756) = Jac_FULL(82,62)
  JVS(756) = B(19)+B(132)+B(158)
! JVS(757) = Jac_FULL(82,63)
  JVS(757) = -B(369)
! JVS(758) = Jac_FULL(82,64)
  JVS(758) = -B(355)
! JVS(759) = Jac_FULL(82,65)
  JVS(759) = -B(357)
! JVS(760) = Jac_FULL(82,67)
  JVS(760) = -B(371)
! JVS(761) = Jac_FULL(82,68)
  JVS(761) = -B(373)
! JVS(762) = Jac_FULL(82,69)
  JVS(762) = -B(363)
! JVS(763) = Jac_FULL(82,70)
  JVS(763) = 0
! JVS(764) = Jac_FULL(82,71)
  JVS(764) = 0.28*B(198)
! JVS(765) = Jac_FULL(82,72)
  JVS(765) = -B(432)
! JVS(766) = Jac_FULL(82,73)
  JVS(766) = 0
! JVS(767) = Jac_FULL(82,74)
  JVS(767) = B(124)+B(154)
! JVS(768) = Jac_FULL(82,75)
  JVS(768) = 0
! JVS(769) = Jac_FULL(82,76)
  JVS(769) = -B(384)
! JVS(770) = Jac_FULL(82,77)
  JVS(770) = -B(382)
! JVS(771) = Jac_FULL(82,78)
  JVS(771) = B(260)+B(341)+B(420)
! JVS(772) = Jac_FULL(82,79)
  JVS(772) = 0.15*B(189)+0.1*B(191)+0.1*B(197)+0.28*B(199)+0.7*B(201)
! JVS(773) = Jac_FULL(82,80)
  JVS(773) = -B(353)
! JVS(774) = Jac_FULL(82,81)
  JVS(774) = 0.23*B(262)+0.12*B(345)-0.88*B(380)+0.23*B(422)
! JVS(775) = Jac_FULL(82,82)
...
! JVS(776) = Jac_FULL(82,83)
  JVS(776) = -B(338)-B(340)+B(342)+0.12*B(346)
! JVS(777) = Jac_FULL(82,84)
  JVS(777) = B(125)+B(133)+0.65*B(145)
! JVS(778) = Jac_FULL(82,85)
  JVS(778) = -B(297)-B(299)
! JVS(779) = Jac_FULL(82,86)
  JVS(779) = 0
! JVS(780) = Jac_FULL(82,87)
  JVS(780) = -B(259)+B(261)+0.23*B(263)+0.25*B(445)
! JVS(781) = Jac_FULL(82,88)
  JVS(781) = -B(225)
! JVS(782) = Jac_FULL(82,89)
  JVS(782) = B(155)+B(159)-B(419)+B(421)+0.23*B(423)
! JVS(783) = Jac_FULL(82,90)
  JVS(783) = -B(352)
! JVS(784) = Jac_FULL(83,26)
  JVS(784) = B(92)
! JVS(785) = Jac_FULL(83,27)
  JVS(785) = 0.65*B(140)
! JVS(786) = Jac_FULL(83,40)
  JVS(786) = B(15)
! JVS(787) = Jac_FULL(83,48)
  JVS(787) = 0.08*B(190)
! JVS(788) = Jac_FULL(83,49)
  JVS(788) = 0.23*B(462)
! JVS(789) = Jac_FULL(83,51)
  JVS(789) = 0.03*B(188)
! JVS(790) = Jac_FULL(83,55)
  JVS(790) = -B(319)+0.5*B(359)
! JVS(791) = Jac_FULL(83,56)
  JVS(791) = -B(321)+0.501*B(361)
! JVS(792) = Jac_FULL(83,58)
  JVS(792) = -B(327)+B(367)
! JVS(793) = Jac_FULL(83,60)
  JVS(793) = -B(325)+0.506*B(365)
! JVS(794) = Jac_FULL(83,61)
  JVS(794) = -B(335)+B(375)
! JVS(795) = Jac_FULL(83,63)
  JVS(795) = -B(329)+B(369)
! JVS(796) = Jac_FULL(83,64)
  JVS(796) = 0.031*B(236)-0.951*B(315)+0.554*B(355)+0.04*B(396)
! JVS(797) = Jac_FULL(83,65)
  JVS(797) = -B(317)+0.507*B(357)
! JVS(798) = Jac_FULL(83,67)
  JVS(798) = -B(331)+B(371)
! JVS(799) = Jac_FULL(83,68)
  JVS(799) = -B(333)+B(373)
! JVS(800) = Jac_FULL(83,69)
  JVS(800) = -B(323)+0.51*B(363)
! JVS(801) = Jac_FULL(83,71)
  JVS(801) = 0
! JVS(802) = Jac_FULL(83,72)
  JVS(802) = -B(430)+B(432)
! JVS(803) = Jac_FULL(83,73)
  JVS(803) = 0.23*B(186)
! JVS(804) = Jac_FULL(83,74)
  JVS(804) = B(12)
! JVS(805) = Jac_FULL(83,75)
  JVS(805) = 0.19*B(184)
! JVS(806) = Jac_FULL(83,76)
  JVS(806) = -B(349)+0.516*B(384)
! JVS(807) = Jac_FULL(83,77)
  JVS(807) = -B(347)+0.5*B(382)
! JVS(808) = Jac_FULL(83,78)
  JVS(808) = -B(341)-B(343)+B(378)
! JVS(809) = Jac_FULL(83,79)
  JVS(809) = 0.19*B(185)+0.23*B(187)+0.03*B(189)+0.08*B(191)+0.23*B(463)
! JVS(810) = Jac_FULL(83,80)
  JVS(810) = 0.15*B(234)-0.993*B(313)+0.508*B(353)+0.155*B(394)
! JVS(811) = Jac_FULL(83,81)
  JVS(811) = -B(345)+0.5*B(380)
! JVS(812) = Jac_FULL(83,82)
...
! JVS(814) = Jac_FULL(83,84)
  JVS(814) = B(93)+0.65*B(141)
! JVS(815) = Jac_FULL(83,85)
  JVS(815) = -B(269)
! JVS(816) = Jac_FULL(83,86)
  JVS(816) = 0
! JVS(817) = Jac_FULL(83,87)
  JVS(817) = -B(231)+0.15*B(235)+0.031*B(237)+B(259)
! JVS(818) = Jac_FULL(83,88)
  JVS(818) = 0
! JVS(819) = Jac_FULL(83,89)
  JVS(819) = -B(391)+0.155*B(395)+0.04*B(397)+B(419)
! JVS(820) = Jac_FULL(83,90)
  JVS(820) = -B(312)+0.5*B(352)
! JVS(821) = Jac_FULL(84,13)
  JVS(821) = 2*B(30)
! JVS(822) = Jac_FULL(84,14)
  JVS(822) = -B(84)
! JVS(823) = Jac_FULL(84,16)
  JVS(823) = -B(116)
! JVS(824) = Jac_FULL(84,17)
  JVS(824) = -B(118)
! JVS(825) = Jac_FULL(84,18)
  JVS(825) = -B(98)
! JVS(826) = Jac_FULL(84,20)
  JVS(826) = -B(100)
! JVS(827) = Jac_FULL(84,21)
  JVS(827) = -B(160)
! JVS(828) = Jac_FULL(84,22)
  JVS(828) = -0.964*B(96)
! JVS(829) = Jac_FULL(84,23)
  JVS(829) = -B(138)
! JVS(830) = Jac_FULL(84,24)
  JVS(830) = -B(88)
! JVS(831) = Jac_FULL(84,25)
  JVS(831) = -B(94)
! JVS(832) = Jac_FULL(84,26)
  JVS(832) = -B(92)
! JVS(833) = Jac_FULL(84,27)
  JVS(833) = B(13)-0.65*B(140)
! JVS(834) = Jac_FULL(84,28)
  JVS(834) = -B(406)
! JVS(835) = Jac_FULL(84,29)
  JVS(835) = 0.35*B(6)-B(66)
! JVS(836) = Jac_FULL(84,30)
  JVS(836) = -B(90)
! JVS(837) = Jac_FULL(84,31)
  JVS(837) = B(4)-B(62)
! JVS(838) = Jac_FULL(84,32)
  JVS(838) = -B(128)
! JVS(839) = Jac_FULL(84,35)
  JVS(839) = 2*B(9)-B(38)
! JVS(840) = Jac_FULL(84,36)
  JVS(840) = B(210)
! JVS(841) = Jac_FULL(84,37)
  JVS(841) = B(216)
! JVS(842) = Jac_FULL(84,38)
  JVS(842) = -B(102)+0.12*B(182)
! JVS(843) = Jac_FULL(84,39)
  JVS(843) = B(222)
! JVS(844) = Jac_FULL(84,40)
  JVS(844) = B(15)-B(144)
! JVS(845) = Jac_FULL(84,41)
  JVS(845) = -B(442)
! JVS(846) = Jac_FULL(84,42)
  JVS(846) = -B(452)+0.63*B(454)
! JVS(847) = Jac_FULL(84,43)
  JVS(847) = B(5)-B(64)
! JVS(848) = Jac_FULL(84,44)
  JVS(848) = -B(146)
! JVS(849) = Jac_FULL(84,45)
  JVS(849) = -B(112)+0.85*B(192)
! JVS(850) = Jac_FULL(84,46)
  JVS(850) = -B(86)
! JVS(851) = Jac_FULL(84,47)
  JVS(851) = -B(114)+0.85*B(194)
! JVS(852) = Jac_FULL(84,48)
  JVS(852) = -B(110)+0.25*B(190)
! JVS(853) = Jac_FULL(84,49)
  JVS(853) = -B(458)+0.63*B(462)
! JVS(854) = Jac_FULL(84,50)
  JVS(854) = -B(120)
! JVS(855) = Jac_FULL(84,51)
  JVS(855) = -B(108)+0.28*B(188)
! JVS(856) = Jac_FULL(84,52)
  JVS(856) = 0
! JVS(857) = Jac_FULL(84,53)
  JVS(857) = -B(130)
! JVS(858) = Jac_FULL(84,54)
  JVS(858) = -B(148)+0.036*B(200)
! JVS(859) = Jac_FULL(84,57)
  JVS(859) = -B(134)+0.19*B(196)
! JVS(860) = Jac_FULL(84,59)
  JVS(860) = -B(126)
! JVS(861) = Jac_FULL(84,60)
  JVS(861) = 0
! JVS(862) = Jac_FULL(84,61)
  JVS(862) = 0
! JVS(863) = Jac_FULL(84,62)
  JVS(863) = -B(132)
! JVS(864) = Jac_FULL(84,63)
  JVS(864) = 0
! JVS(865) = Jac_FULL(84,64)
  JVS(865) = 0
! JVS(866) = Jac_FULL(84,65)
  JVS(866) = 0
! JVS(867) = Jac_FULL(84,66)
  JVS(867) = -B(122)
! JVS(868) = Jac_FULL(84,67)
  JVS(868) = 0
! JVS(869) = Jac_FULL(84,68)
  JVS(869) = 0
! JVS(870) = Jac_FULL(84,69)
  JVS(870) = 0
! JVS(871) = Jac_FULL(84,70)
  JVS(871) = -B(150)
! JVS(872) = Jac_FULL(84,71)
  JVS(872) = -B(136)+0.21*B(198)
! JVS(873) = Jac_FULL(84,72)
  JVS(873) = 0
! JVS(874) = Jac_FULL(84,73)
  JVS(874) = -B(106)+0.63*B(186)
! JVS(875) = Jac_FULL(84,74)
  JVS(875) = -B(124)
! JVS(876) = Jac_FULL(84,75)
  JVS(876) = -B(104)+0.4*B(184)
! JVS(877) = Jac_FULL(84,76)
  JVS(877) = 0
! JVS(878) = Jac_FULL(84,77)
  JVS(878) = 0
! JVS(879) = Jac_FULL(84,78)
  JVS(879) = 0
! JVS(880) = Jac_FULL(84,79)
...
! JVS(881) = Jac_FULL(84,80)
  JVS(881) = 0
! JVS(882) = Jac_FULL(84,81)
  JVS(882) = 0
! JVS(883) = Jac_FULL(84,82)
  JVS(883) = 0
! JVS(884) = Jac_FULL(84,83)
  JVS(884) = 0
! JVS(885) = Jac_FULL(84,84)
...
! JVS(886) = Jac_FULL(84,85)
  JVS(886) = B(35)-B(37)+B(55)+0.7*B(60)
! JVS(887) = Jac_FULL(84,86)
  JVS(887) = B(14)-0.51*B(143)
! JVS(888) = Jac_FULL(84,87)
  JVS(888) = -B(50)+B(56)
! JVS(889) = Jac_FULL(84,88)
  JVS(889) = -B(52)
! JVS(890) = Jac_FULL(84,89)
  JVS(890) = -B(54)+0.7*B(61)
! JVS(891) = Jac_FULL(84,90)
  JVS(891) = 0
! JVS(892) = Jac_FULL(85,14)
  JVS(892) = B(84)
! JVS(893) = Jac_FULL(85,16)
  JVS(893) = 0.1*B(116)
! JVS(894) = Jac_FULL(85,17)
  JVS(894) = 0.1*B(118)
! JVS(895) = Jac_FULL(85,18)
  JVS(895) = 0.25*B(98)
! JVS(896) = Jac_FULL(85,20)
  JVS(896) = 0.049*B(100)
! JVS(897) = Jac_FULL(85,22)
  JVS(897) = 0.381*B(96)
! JVS(898) = Jac_FULL(85,23)
  JVS(898) = B(138)
! JVS(899) = Jac_FULL(85,27)
  JVS(899) = B(13)
! JVS(900) = Jac_FULL(85,29)
  JVS(900) = 0.65*B(6)+B(59)
! JVS(901) = Jac_FULL(85,30)
  JVS(901) = B(90)
! JVS(902) = Jac_FULL(85,32)
  JVS(902) = B(23)+B(128)
! JVS(903) = Jac_FULL(85,34)
  JVS(903) = -B(204)
! JVS(904) = Jac_FULL(85,35)
  JVS(904) = B(38)
! JVS(905) = Jac_FULL(85,36)
  JVS(905) = 0.02*B(208)
! JVS(906) = Jac_FULL(85,37)
  JVS(906) = 0.02*B(214)
! JVS(907) = Jac_FULL(85,38)
  JVS(907) = 0.26*B(182)
! JVS(908) = Jac_FULL(85,39)
  JVS(908) = 0.02*B(220)
! JVS(909) = Jac_FULL(85,40)
  JVS(909) = 0.35*B(144)
! JVS(910) = Jac_FULL(85,41)
  JVS(910) = 0
! JVS(911) = Jac_FULL(85,45)
  JVS(911) = 0.1*B(192)
! JVS(912) = Jac_FULL(85,46)
  JVS(912) = B(86)
! JVS(913) = Jac_FULL(85,47)
  JVS(913) = 0.1*B(194)
! JVS(914) = Jac_FULL(85,48)
  JVS(914) = 0.25*B(190)
! JVS(915) = Jac_FULL(85,49)
  JVS(915) = 0.22*B(462)
! JVS(916) = Jac_FULL(85,50)
  JVS(916) = 0.05*B(120)
! JVS(917) = Jac_FULL(85,51)
  JVS(917) = 0.3*B(188)
! JVS(918) = Jac_FULL(85,52)
  JVS(918) = 0.75*B(444)-B(446)+B(448)
! JVS(919) = Jac_FULL(85,53)
  JVS(919) = 0.8*B(18)+B(130)+B(156)
! JVS(920) = Jac_FULL(85,54)
  JVS(920) = 0.4*B(148)+0.08*B(200)
! JVS(921) = Jac_FULL(85,55)
  JVS(921) = B(240)-B(278)+B(319)+0.5*B(359)+B(400)
! JVS(922) = Jac_FULL(85,56)
  JVS(922) = B(242)-B(280)+B(321)+0.501*B(361)+B(402)
! JVS(923) = Jac_FULL(85,57)
  JVS(923) = B(22)+0.32*B(196)
! JVS(924) = Jac_FULL(85,58)
  JVS(924) = 0.8*B(248)-B(286)+2*B(327)+B(367)+B(408)
! JVS(925) = Jac_FULL(85,60)
  JVS(925) = B(246)-B(284)+B(325)+0.506*B(365)+B(439)
! JVS(926) = Jac_FULL(85,61)
  JVS(926) = B(256)-B(294)+2*B(335)+B(375)+B(416)
! JVS(927) = Jac_FULL(85,62)
  JVS(927) = B(19)
! JVS(928) = Jac_FULL(85,63)
  JVS(928) = 0.65*B(250)-B(288)+2*B(329)+B(369)+B(410)
! JVS(929) = Jac_FULL(85,64)
  JVS(929) = 0.599*B(236)-B(274)+0.946*B(315)+0.438*B(355)+0.699*B(396)
! JVS(930) = Jac_FULL(85,65)
  JVS(930) = 0.606*B(238)-B(276)+0.993*B(317)+0.489*B(357)+0.845*B(398)
! JVS(931) = Jac_FULL(85,66)
  JVS(931) = 2*B(11)+B(122)+B(152)
! JVS(932) = Jac_FULL(85,67)
  JVS(932) = 0.95*B(252)-B(290)+B(331)+B(371)+B(412)
! JVS(933) = Jac_FULL(85,68)
  JVS(933) = 0.95*B(254)-B(292)+B(333)+B(373)+B(414)
! JVS(934) = Jac_FULL(85,69)
  JVS(934) = B(244)-B(282)+B(323)+0.51*B(363)+B(404)
! JVS(935) = Jac_FULL(85,70)
  JVS(935) = B(21)
! JVS(936) = Jac_FULL(85,71)
  JVS(936) = B(20)+0.5*B(136)+0.5*B(162)+0.29*B(198)
! JVS(937) = Jac_FULL(85,72)
  JVS(937) = -B(428)+B(430)
! JVS(938) = Jac_FULL(85,73)
  JVS(938) = 0.22*B(186)
! JVS(939) = Jac_FULL(85,74)
  JVS(939) = B(12)
! JVS(940) = Jac_FULL(85,75)
  JVS(940) = 0.25*B(184)
! JVS(941) = Jac_FULL(85,76)
  JVS(941) = -B(308)+0.5*B(349)+0.5*B(387)
! JVS(942) = Jac_FULL(85,77)
  JVS(942) = B(264)-B(306)+B(347)+0.5*B(382)+B(386)+0.5*B(388)+B(424)
! JVS(943) = Jac_FULL(85,78)
  JVS(943) = -B(300)-B(302)+B(341)
! JVS(944) = Jac_FULL(85,79)
...
! JVS(950) = Jac_FULL(85,85)
  JVS(950) = -B(35)-B(37)-2*B(40)-2*B(41)-B(55)-B(57)-B(60)-B(205)-B(269)-B(270)-B(273)-B(275)-B(277)-B(279)-B(281)&
               ...
               &-B(447)
! JVS(951) = Jac_FULL(85,86)
  JVS(951) = B(14)
! JVS(952) = Jac_FULL(85,87)
...
! JVS(953) = Jac_FULL(85,88)
  JVS(953) = -B(58)
! JVS(954) = Jac_FULL(85,89)
...
! JVS(955) = Jac_FULL(85,90)
  JVS(955) = B(233)-B(271)+B(312)+0.5*B(352)+B(393)
! JVS(956) = Jac_FULL(86,55)
  JVS(956) = B(278)
! JVS(957) = Jac_FULL(86,56)
  JVS(957) = B(280)
! JVS(958) = Jac_FULL(86,58)
  JVS(958) = B(286)
! JVS(959) = Jac_FULL(86,61)
  JVS(959) = B(294)
! JVS(960) = Jac_FULL(86,63)
  JVS(960) = B(288)
! JVS(961) = Jac_FULL(86,64)
  JVS(961) = B(274)
! JVS(962) = Jac_FULL(86,65)
  JVS(962) = B(276)
! JVS(963) = Jac_FULL(86,67)
  JVS(963) = B(290)
! JVS(964) = Jac_FULL(86,68)
  JVS(964) = B(292)
! JVS(965) = Jac_FULL(86,69)
  JVS(965) = B(282)
! JVS(966) = Jac_FULL(86,72)
  JVS(966) = B(428)
! JVS(967) = Jac_FULL(86,73)
  JVS(967) = 0
! JVS(968) = Jac_FULL(86,75)
  JVS(968) = 0
! JVS(969) = Jac_FULL(86,78)
  JVS(969) = B(300)
! JVS(970) = Jac_FULL(86,79)
  JVS(970) = 0
! JVS(971) = Jac_FULL(86,80)
  JVS(971) = B(272)
! JVS(972) = Jac_FULL(86,81)
  JVS(972) = B(304)
! JVS(973) = Jac_FULL(86,82)
  JVS(973) = 0
! JVS(974) = Jac_FULL(86,83)
  JVS(974) = 0
! JVS(975) = Jac_FULL(86,84)
  JVS(975) = -B(142)
! JVS(976) = Jac_FULL(86,85)
  JVS(976) = B(270)+B(273)+B(275)+B(277)+B(279)+B(281)+B(283)+B(287)+B(289)+B(291)+B(293)+B(295)+B(301)+B(305)+B(429)
! JVS(977) = Jac_FULL(86,86)
  JVS(977) = -B(14)-B(143)
! JVS(978) = Jac_FULL(86,87)
  JVS(978) = 0
! JVS(979) = Jac_FULL(86,88)
  JVS(979) = 0
! JVS(980) = Jac_FULL(86,89)
  JVS(980) = 0
! JVS(981) = Jac_FULL(86,90)
  JVS(981) = B(271)
! JVS(982) = Jac_FULL(87,31)
  JVS(982) = B(4)
! JVS(983) = Jac_FULL(87,33)
  JVS(983) = -B(43)+B(45)
! JVS(984) = Jac_FULL(87,36)
  JVS(984) = 0
! JVS(985) = Jac_FULL(87,37)
  JVS(985) = 0
! JVS(986) = Jac_FULL(87,39)
  JVS(986) = 0
! JVS(987) = Jac_FULL(87,50)
  JVS(987) = 0
! JVS(988) = Jac_FULL(87,51)
  JVS(988) = 0
! JVS(989) = Jac_FULL(87,52)
  JVS(989) = -B(444)
! JVS(990) = Jac_FULL(87,55)
  JVS(990) = -B(240)
! JVS(991) = Jac_FULL(87,56)
  JVS(991) = -B(242)
! JVS(992) = Jac_FULL(87,57)
  JVS(992) = 0
! JVS(993) = Jac_FULL(87,58)
  JVS(993) = -B(248)
! JVS(994) = Jac_FULL(87,60)
  JVS(994) = -B(246)
! JVS(995) = Jac_FULL(87,61)
  JVS(995) = -B(256)
! JVS(996) = Jac_FULL(87,63)
  JVS(996) = -B(250)
! JVS(997) = Jac_FULL(87,64)
  JVS(997) = -B(236)
! JVS(998) = Jac_FULL(87,65)
  JVS(998) = -B(238)
! JVS(999) = Jac_FULL(87,67)
  JVS(999) = -B(252)
! JVS(1000) = Jac_FULL(87,68)
  JVS(1000) = -B(254)
! JVS(1001) = Jac_FULL(87,69)
  JVS(1001) = -B(244)
! JVS(1002) = Jac_FULL(87,72)
  JVS(1002) = -B(435)
! JVS(1003) = Jac_FULL(87,73)
  JVS(1003) = 0
! JVS(1004) = Jac_FULL(87,75)
  JVS(1004) = 0
! JVS(1005) = Jac_FULL(87,76)
  JVS(1005) = -B(266)
! JVS(1006) = Jac_FULL(87,77)
  JVS(1006) = -B(264)
! JVS(1007) = Jac_FULL(87,78)
  JVS(1007) = -B(260)
! JVS(1008) = Jac_FULL(87,79)
  JVS(1008) = -B(68)
! JVS(1009) = Jac_FULL(87,80)
  JVS(1009) = -B(234)
! JVS(1010) = Jac_FULL(87,81)
  JVS(1010) = -B(262)
! JVS(1011) = Jac_FULL(87,82)
  JVS(1011) = -B(258)
! JVS(1012) = Jac_FULL(87,83)
  JVS(1012) = -B(230)
! JVS(1013) = Jac_FULL(87,84)
  JVS(1013) = -B(49)
! JVS(1014) = Jac_FULL(87,85)
  JVS(1014) = -B(55)
! JVS(1015) = Jac_FULL(87,86)
  JVS(1015) = 0
! JVS(1016) = Jac_FULL(87,87)
...
! JVS(1017) = Jac_FULL(87,88)
  JVS(1017) = B(1)+B(46)+B(76)
! JVS(1018) = Jac_FULL(87,89)
  JVS(1018) = B(7)-B(75)+B(77)
! JVS(1019) = Jac_FULL(87,90)
  JVS(1019) = -B(233)
! JVS(1020) = Jac_FULL(88,19)
  JVS(1020) = B(80)
! JVS(1021) = Jac_FULL(88,24)
  JVS(1021) = B(88)
! JVS(1022) = Jac_FULL(88,28)
  JVS(1022) = B(406)+B(451)
! JVS(1023) = Jac_FULL(88,29)
  JVS(1023) = 0.65*B(6)+B(59)+B(66)
! JVS(1024) = Jac_FULL(88,31)
  JVS(1024) = B(62)
! JVS(1025) = Jac_FULL(88,33)
  JVS(1025) = B(43)-B(45)-B(47)
! JVS(1026) = Jac_FULL(88,34)
  JVS(1026) = -B(202)
! JVS(1027) = Jac_FULL(88,36)
  JVS(1027) = -B(206)
! JVS(1028) = Jac_FULL(88,37)
  JVS(1028) = -B(212)
! JVS(1029) = Jac_FULL(88,39)
  JVS(1029) = -B(218)
! JVS(1030) = Jac_FULL(88,41)
  JVS(1030) = 0
! JVS(1031) = Jac_FULL(88,43)
  JVS(1031) = B(5)
! JVS(1032) = Jac_FULL(88,44)
  JVS(1032) = B(226)
! JVS(1033) = Jac_FULL(88,48)
  JVS(1033) = 0
! JVS(1034) = Jac_FULL(88,50)
  JVS(1034) = 0
! JVS(1035) = Jac_FULL(88,51)
  JVS(1035) = 0
! JVS(1036) = Jac_FULL(88,52)
  JVS(1036) = B(444)-B(449)
! JVS(1037) = Jac_FULL(88,53)
  JVS(1037) = 0
! JVS(1038) = Jac_FULL(88,54)
  JVS(1038) = 0.4*B(180)+0.7*B(200)+B(229)
! JVS(1039) = Jac_FULL(88,55)
  JVS(1039) = B(240)+B(400)
! JVS(1040) = Jac_FULL(88,56)
  JVS(1040) = B(242)+B(402)
! JVS(1041) = Jac_FULL(88,57)
  JVS(1041) = 0
! JVS(1042) = Jac_FULL(88,58)
  JVS(1042) = 0.8*B(248)+B(408)
! JVS(1043) = Jac_FULL(88,60)
  JVS(1043) = B(246)
! JVS(1044) = Jac_FULL(88,61)
  JVS(1044) = B(256)+B(416)
! JVS(1045) = Jac_FULL(88,62)
  JVS(1045) = 0
! JVS(1046) = Jac_FULL(88,63)
  JVS(1046) = 0.65*B(250)+B(410)
! JVS(1047) = Jac_FULL(88,64)
  JVS(1047) = 0.876*B(236)+B(396)
! JVS(1048) = Jac_FULL(88,65)
  JVS(1048) = 0.739*B(238)+B(398)
! JVS(1049) = Jac_FULL(88,66)
  JVS(1049) = 0
! JVS(1050) = Jac_FULL(88,67)
  JVS(1050) = 0.95*B(252)+B(412)
! JVS(1051) = Jac_FULL(88,68)
  JVS(1051) = 0.95*B(254)+B(414)
! JVS(1052) = Jac_FULL(88,69)
  JVS(1052) = B(244)+B(404)
! JVS(1053) = Jac_FULL(88,70)
  JVS(1053) = B(21)+B(150)
! JVS(1054) = Jac_FULL(88,71)
  JVS(1054) = 0.5*B(162)
! JVS(1055) = Jac_FULL(88,72)
  JVS(1055) = B(435)+B(437)
! JVS(1056) = Jac_FULL(88,73)
  JVS(1056) = 0
! JVS(1057) = Jac_FULL(88,74)
  JVS(1057) = 0
! JVS(1058) = Jac_FULL(88,75)
  JVS(1058) = 0
! JVS(1059) = Jac_FULL(88,76)
  JVS(1059) = 2*B(266)+0.5*B(349)+0.516*B(384)+0.5*B(387)+B(389)+2*B(426)
! JVS(1060) = Jac_FULL(88,77)
  JVS(1060) = B(264)+0.5*B(388)+B(424)
! JVS(1061) = Jac_FULL(88,78)
  JVS(1061) = -B(227)+B(260)+B(420)
! JVS(1062) = Jac_FULL(88,79)
  JVS(1062) = B(68)-B(70)+0.7*B(201)
! JVS(1063) = Jac_FULL(88,80)
  JVS(1063) = 0.941*B(234)+B(394)
! JVS(1064) = Jac_FULL(88,81)
  JVS(1064) = B(262)+B(422)
! JVS(1065) = Jac_FULL(88,82)
  JVS(1065) = -B(224)+B(258)+0.516*B(385)+B(418)
! JVS(1066) = Jac_FULL(88,83)
  JVS(1066) = B(230)+0.5*B(350)+B(390)
! JVS(1067) = Jac_FULL(88,84)
  JVS(1067) = -B(51)+B(53)+B(63)+B(67)+B(89)+B(151)+B(407)
! JVS(1068) = Jac_FULL(88,85)
  JVS(1068) = B(55)-B(57)+0.7*B(60)
! JVS(1069) = Jac_FULL(88,86)
  JVS(1069) = 0
! JVS(1070) = Jac_FULL(88,87)
...
! JVS(1073) = Jac_FULL(88,90)
  JVS(1073) = B(233)+B(393)
! JVS(1074) = Jac_FULL(89,19)
  JVS(1074) = B(80)
! JVS(1075) = Jac_FULL(89,29)
  JVS(1075) = 0.35*B(6)
! JVS(1076) = Jac_FULL(89,33)
  JVS(1076) = B(47)
! JVS(1077) = Jac_FULL(89,38)
  JVS(1077) = -B(166)
! JVS(1078) = Jac_FULL(89,42)
  JVS(1078) = -B(456)
! JVS(1079) = Jac_FULL(89,43)
  JVS(1079) = B(64)
! JVS(1080) = Jac_FULL(89,44)
  JVS(1080) = B(146)
! JVS(1081) = Jac_FULL(89,45)
  JVS(1081) = -B(176)
! JVS(1082) = Jac_FULL(89,47)
  JVS(1082) = -B(178)
! JVS(1083) = Jac_FULL(89,48)
  JVS(1083) = -B(174)
! JVS(1084) = Jac_FULL(89,49)
  JVS(1084) = -B(460)
! JVS(1085) = Jac_FULL(89,50)
  JVS(1085) = -B(164)
! JVS(1086) = Jac_FULL(89,51)
  JVS(1086) = -B(172)
! JVS(1087) = Jac_FULL(89,53)
  JVS(1087) = -B(156)
! JVS(1088) = Jac_FULL(89,54)
  JVS(1088) = 0.6*B(148)-0.4*B(180)
! JVS(1089) = Jac_FULL(89,55)
  JVS(1089) = -B(400)
! JVS(1090) = Jac_FULL(89,56)
  JVS(1090) = -B(402)
! JVS(1091) = Jac_FULL(89,58)
  JVS(1091) = -B(408)
! JVS(1092) = Jac_FULL(89,61)
  JVS(1092) = -B(416)
! JVS(1093) = Jac_FULL(89,62)
  JVS(1093) = -B(158)
! JVS(1094) = Jac_FULL(89,63)
  JVS(1094) = -B(410)
! JVS(1095) = Jac_FULL(89,64)
  JVS(1095) = -B(396)
! JVS(1096) = Jac_FULL(89,65)
  JVS(1096) = -B(398)
! JVS(1097) = Jac_FULL(89,66)
  JVS(1097) = -B(152)
! JVS(1098) = Jac_FULL(89,67)
  JVS(1098) = -B(412)
! JVS(1099) = Jac_FULL(89,68)
  JVS(1099) = -B(414)
! JVS(1100) = Jac_FULL(89,69)
  JVS(1100) = -B(404)
! JVS(1101) = Jac_FULL(89,71)
  JVS(1101) = -B(162)
! JVS(1102) = Jac_FULL(89,72)
  JVS(1102) = -B(437)
! JVS(1103) = Jac_FULL(89,73)
  JVS(1103) = -B(170)
! JVS(1104) = Jac_FULL(89,74)
  JVS(1104) = -B(154)
! JVS(1105) = Jac_FULL(89,75)
  JVS(1105) = -B(168)
! JVS(1106) = Jac_FULL(89,76)
  JVS(1106) = -B(426)
! JVS(1107) = Jac_FULL(89,77)
  JVS(1107) = -B(424)
! JVS(1108) = Jac_FULL(89,78)
  JVS(1108) = -B(420)
! JVS(1109) = Jac_FULL(89,79)
  JVS(1109) = B(70)
! JVS(1110) = Jac_FULL(89,80)
  JVS(1110) = -B(394)
! JVS(1111) = Jac_FULL(89,81)
  JVS(1111) = -B(422)
! JVS(1112) = Jac_FULL(89,82)
  JVS(1112) = -B(418)
! JVS(1113) = Jac_FULL(89,83)
  JVS(1113) = -B(390)
! JVS(1114) = Jac_FULL(89,84)
  JVS(1114) = -B(53)+B(65)+B(147)+0.6*B(149)
! JVS(1115) = Jac_FULL(89,85)
  JVS(1115) = -B(60)
! JVS(1116) = Jac_FULL(89,86)
  JVS(1116) = 0
! JVS(1117) = Jac_FULL(89,87)
  JVS(1117) = -B(74)
! JVS(1118) = Jac_FULL(89,88)
  JVS(1118) = B(48)+B(71)-B(76)-B(78)
! JVS(1119) = Jac_FULL(89,89)
...
! JVS(1120) = Jac_FULL(89,90)
  JVS(1120) = -B(393)
! JVS(1121) = Jac_FULL(90,25)
  JVS(1121) = B(94)
! JVS(1122) = Jac_FULL(90,45)
  JVS(1122) = 0.2*B(192)
! JVS(1123) = Jac_FULL(90,47)
  JVS(1123) = 0.16*B(194)
! JVS(1124) = Jac_FULL(90,49)
  JVS(1124) = 0.18*B(462)
! JVS(1125) = Jac_FULL(90,59)
  JVS(1125) = B(16)
! JVS(1126) = Jac_FULL(90,64)
  JVS(1126) = 0.245*B(236)+0.014*B(315)+0.018*B(355)+0.262*B(396)
! JVS(1127) = Jac_FULL(90,65)
  JVS(1127) = 0.133*B(238)+0.013*B(317)+0.015*B(357)+0.155*B(398)
! JVS(1128) = Jac_FULL(90,69)
  JVS(1128) = 0
! JVS(1129) = Jac_FULL(90,70)
  JVS(1129) = 0
! JVS(1130) = Jac_FULL(90,71)
  JVS(1130) = 0
! JVS(1131) = Jac_FULL(90,73)
  JVS(1131) = 0.18*B(186)
! JVS(1132) = Jac_FULL(90,75)
  JVS(1132) = 0.1*B(184)
! JVS(1133) = Jac_FULL(90,76)
  JVS(1133) = 0
! JVS(1134) = Jac_FULL(90,77)
  JVS(1134) = 0
! JVS(1135) = Jac_FULL(90,78)
  JVS(1135) = 0
! JVS(1136) = Jac_FULL(90,79)
  JVS(1136) = 0.1*B(185)+0.18*B(187)+0.2*B(193)+0.16*B(195)+0.18*B(463)
! JVS(1137) = Jac_FULL(90,80)
  JVS(1137) = 0.048*B(234)+0.006*B(353)+0.053*B(394)
! JVS(1138) = Jac_FULL(90,81)
  JVS(1138) = 0
! JVS(1139) = Jac_FULL(90,82)
  JVS(1139) = -B(351)+0.006*B(354)+0.018*B(356)+0.015*B(358)
! JVS(1140) = Jac_FULL(90,83)
  JVS(1140) = -B(311)+0.014*B(316)+0.013*B(318)
! JVS(1141) = Jac_FULL(90,84)
  JVS(1141) = B(95)
! JVS(1142) = Jac_FULL(90,85)
  JVS(1142) = -B(270)
! JVS(1143) = Jac_FULL(90,86)
  JVS(1143) = 0
! JVS(1144) = Jac_FULL(90,87)
  JVS(1144) = -B(232)+0.048*B(235)+0.245*B(237)+0.133*B(239)
! JVS(1145) = Jac_FULL(90,88)
  JVS(1145) = 0
! JVS(1146) = Jac_FULL(90,89)
  JVS(1146) = -B(392)+0.053*B(395)+0.262*B(397)+0.155*B(399)
! JVS(1147) = Jac_FULL(90,90)
  JVS(1147) = -B(233)-B(271)-B(312)-B(352)-B(393)
      
END SUBROUTINE racm_soa_vbs_Jac_SP

! End of racm_soa_vbs_Jac_SP function
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE racm_soa_vbs_KppDecomp( JVS, IER )
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
END SUBROUTINE racm_soa_vbs_KppDecomp


! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE racm_soa_vbs_KppDecompCmplx( JVS, IER )
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
END SUBROUTINE racm_soa_vbs_KppDecompCmplx

! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE racm_soa_vbs_KppSolveIndirect( JVS, X )
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
END SUBROUTINE racm_soa_vbs_KppSolveIndirect

! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE racm_soa_vbs_KppSolveCmplx( JVS, X )
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
END SUBROUTINE racm_soa_vbs_KppSolveCmplx
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
! racm_soa_vbs_KppSolve - sparse back substitution
!   Arguments :
!      JVS       - sparse Jacobian of variables
...
SUBROUTINE racm_soa_vbs_KppSolve ( JVS, X )
...
END SUBROUTINE racm_soa_vbs_KppSolve

! End of racm_soa_vbs_KppSolve function
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!--------------------------------------------------------------
      SUBROUTINE racm_soa_vbs_WCOPY(N,X,incX,Y,incY)
!--------------------------------------------------------------
...
!     USE racm_soa_vbs_Precision
      ...
      END SUBROUTINE racm_soa_vbs_WCOPY


!--------------------------------------------------------------
      SUBROUTINE racm_soa_vbs_WAXPY(N,Alpha,X,incX,Y,incY)
!--------------------------------------------------------------
...
!      USE racm_soa_vbs_Precision
      ...
      END SUBROUTINE racm_soa_vbs_WAXPY



!--------------------------------------------------------------
      SUBROUTINE racm_soa_vbs_WSCAL(N,Alpha,X,incX)
!--------------------------------------------------------------
...
!      USE racm_soa_vbs_Precision
      ...
      END SUBROUTINE racm_soa_vbs_WSCAL

!--------------------------------------------------------------
      REAL(kind=dp) FUNCTION racm_soa_vbs_WLAMCH( C )
!--------------------------------------------------------------
...
!      USE racm_soa_vbs_Precision
      ...
      IF (First) THEN
        ...
        DO i = 17, 80
          Eps = Eps*HALF
          CALL racm_soa_vbs_WLAMCH_ADD(ONE,Eps,Sum)
...
10      Eps = Eps*2
      ...
      racm_soa_vbs_WLAMCH = Eps

      END FUNCTION racm_soa_vbs_WLAMCH
     
      SUBROUTINE racm_soa_vbs_WLAMCH_ADD( A, B, Sum )
!      USE racm_soa_vbs_Precision
      
      REAL(kind=dp) A, B, Sum
      Sum = A + B

      END SUBROUTINE racm_soa_vbs_WLAMCH_ADD
!--------------------------------------------------------------


!--------------------------------------------------------------
      SUBROUTINE racm_soa_vbs_SET2ZERO(N,Y)
!--------------------------------------------------------------
...
!--------------------------------------------------------------
      ...
      END SUBROUTINE racm_soa_vbs_SET2ZERO


!--------------------------------------------------------------
      REAL(kind=dp) FUNCTION racm_soa_vbs_WDOT (N, DX, incX, DY, incY) 
!--------------------------------------------------------------
...
!                                                                       
    5 IX = 1 
      ...
      DO i = 1,N 
        racm_soa_vbs_WDOT = racm_soa_vbs_WDOT + DX(IX)*DY(IY) 
        IX = IX + incX 
        IY = IY + incY 
      END DO 
      RETURN 
...
!                                                                       
   20 M = MOD(N,5) 
      IF (M .EQ. 0) GO TO 40 
      DO i = 1,M 
         racm_soa_vbs_WDOT = racm_soa_vbs_WDOT + DX(i)*DY(i) 
      END DO 
      IF (N .LT. 5) RETURN 
   40 MP1 = M + 1 
      DO i = MP1,N,5 
          racm_soa_vbs_WDOT = racm_soa_vbs_WDOT + DX(i)*DY(i) + DX(i+1)*DY(i+1) +&
                   DX(i+2)*DY(i+2) +  &
...
!                                                                       
   60 NS = N*incX 
      ...
      END FUNCTION racm_soa_vbs_WDOT                                          
! End of BLAS_UTIL function
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


   SUBROUTINE decomp_racm_soa_vbs( JVS, IER )
  ...
  JVS( 1147) = W( 90 )
   ...
   END SUBROUTINE decomp_racm_soa_vbs
