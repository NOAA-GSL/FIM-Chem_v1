MODULE module_racmsoavbs_driver
  USE racm_soa_vbs_Parameters
  USE racm_soa_vbs_Precision
  USE racm_soa_vbs_UpdateRconstWRF
  USE racm_soa_vbs_Integrator
...
SUBROUTINE  racm_soa_vbs_driver( &
...
REAL( KIND = dp ) :: rc_n2o5
   ...
   RCNTRL(3) = 0.01_dp * TIME_END
    ...
    var(ind_HACE) = conv  * REAL( MAX(chem(i,k,j,P_hace),0.), KIND=dp)  
    var(ind_ISHP) = conv  * REAL( MAX(chem(i,k,j,P_ishp),0.), KIND=dp)  
    var(ind_ISON) = conv  * REAL( MAX(chem(i,k,j,P_ison),0.), KIND=dp)  
    var(ind_MACP) =  conv * REAL( MAX(macp(i,k,j),0.), KIND=dp)  
    var(ind_MAHP) = conv  * REAL( MAX(chem(i,k,j,P_mahp),0.), KIND=dp)  
    var(ind_MPAN) = conv  * REAL( MAX(chem(i,k,j,P_mpan),0.), KIND=dp)  
    var(ind_NALD) = conv  * REAL( MAX(chem(i,k,j,P_nald),0.), KIND=dp)  
   ...
   CALL racm_soa_vbs_Update_Rconst(  &
  ...
  CALL racm_soa_vbs_INTEGRATE(TIME_START, TIME_END, &  
...
if(p_nu0.gt.1)then
   ...
   xo2(i,k,j) = MAX (REAL (oconv * var(ind_XO2) , KIND=sp),0.) 
    chem(i,k,j,P_hace) = MAX ( REAL (oconv * var(ind_HACE), KIND=sp), 0.)  
    chem(i,k,j,P_ishp) = MAX ( REAL (oconv * var(ind_ISHP), KIND=sp), 0.)  
    chem(i,k,j,P_ison) = MAX ( REAL (oconv * var(ind_ISON), KIND=sp), 0.)  
   macp(i,k,j) = MAX (REAL (oconv * var(ind_MACP) , KIND=sp),0.) 
    chem(i,k,j,P_mahp) = MAX ( REAL (oconv * var(ind_MAHP), KIND=sp), 0.)  
    chem(i,k,j,P_mpan) = MAX ( REAL (oconv * var(ind_MPAN), KIND=sp), 0.)  
    chem(i,k,j,P_nald) = MAX ( REAL (oconv * var(ind_NALD), KIND=sp), 0.)  
...
END SUBROUTINE  racm_soa_vbs_driver


END MODULE module_racmsoavbs_driver
