...
!  ---  outputs:
      real (kind=kind_phys), intent(inout) :: slag, sdec, cdec, solcon
      real (kind=kind_phys), intent(inout) :: anginc, sindec, cosdec
      real (kind=kind_phys), intent(inout) :: sollag
...
!  ---  outputs: (horizontal dimensioned by ix)
      real (kind=kind_phys),dimension(ix,lm),intent(inout)::htrsw,htrlw,&
     &       cldcov
     ...
     &       sfalb, semis, coszen, coszdg
      ...
      type (topflw_type), dimension(im), intent(inout) :: topflw
      type (sfcflw_type), dimension(im), intent(inout) :: sfcflw

!JR Variables moved to stack for OMP
      integer, intent(inout) :: nstp
      real (kind=kind_phys), intent(inout) :: anginc
      real (kind=kind_phys), intent(inout) :: sindec
      real (kind=kind_phys), intent(inout) :: cosdec
      real (kind=kind_phys), intent(inout) :: sollag
...
!!    type (proflw_type),    dimension(im,lm+1+ltp) :: flwprf
      ...
      real (kind=kind_phys)  sscal(lm,14)
      real (kind=kind_phys)  asymp(lm,14)
      real (kind=kind_phys)  ext_cof(lm,14)
      real (kind=kind_phys)  extlw_cof(lm,16)
...
!    &       faersw,faerlw,aerodp                                       &
     &     )
       if(sscal(1,1).lt.100)then
!      write(6,*)'overwriting aerosol optical props',im,ix
        do k = 1, LM
          do i = 1, IM
            faerlw(i,k,:,1)=extlw_cof(k,:)
            faersw(i,k,:,1)=ext_cof(k,:)
            faersw(i,k,:,2)=sscal(k,:)
            faersw(i,k,:,3)=asymp(k,:)
          enddo
        enddo
        else
        do k = 1, LM
          do i = 1, IM
            extlw_cof(k,:)=faerlw(i,k,:,1)
            ext_cof(k,:)=faersw(i,k,:,1)
            sscal(k,:)=faersw(i,k,:,2)
            asymp(k,:)=faersw(i,k,:,3)
          enddo
        enddo
        endif
...
!  ---  save total cloud and bl cloud
        ...
        if (lsswr .or. lslwr) then
          do i = 1, im
!ss         fluxr(i,17) = fluxr(i,17) + raddt * cldsa(i,4)
!ss         fluxr(i,18) = fluxr(i,18) + raddt * cldsa(i,5)
            fluxr(i,17) = cldsa(i,4)  !ss total cloud
            fluxr(i,18) = cldsa(i,5)  !ss bl cloud
          enddo
...
!  ---  save interface pressure (cb) of top/bot
          ...
          do j = 1, 3
            do i = 1, im
              ...
              itop  = mtopa(i,j) - kd
              ibtc  = mbota(i,j) - kd
!ss           fluxr(i, 8-j) = fluxr(i, 8-j) + tem0d
              fluxr(i, 8-j) = cldsa(i,j)  ! ss 5,6,7-> h,m,l cloud
