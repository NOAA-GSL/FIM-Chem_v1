...
!-------------------------------------------------------------
   ...
   INTEGER,      INTENT(IN   ) :: numc,ipn,IX,KM,lonf,ntrac,tottracer
...
!       massflni,xfi_ens,pri_ens
     ...
     real,    dimension (ix,km,numc) ::                    &
        outc,outcs,outcm,chem_pwdout,chem_pwout
     real,    dimension (ix,km) ::                    &
...
!
   ...
   INTEGER :: i,j,k,nv,ICLDCK,ipr,jpr,jpr_deep,ipr_deep
...
!
   call CUP_gf_sh(cnvwts,zus,zws,xmbs,xmbs2,zo,OUTQCs,ipn,AAEQ,T2D,Q2D,TER11, &
           chem,outcs,prets,Tshall,Qshall,P2d,P2d,OUTTS,OUTQS,DT,PSUR,US,VS,  &
       ...
       enddo
       CALL neg_check(ktops,'shallow',ipn,dt,qcheck,outqs,outts,outus,outvs,   &
                      outqcs,outcs,prets,its,ite,kts,kte,itf,ktf,numc)
       endif
...
!            enddo
!            enddo
      CALL CUP_gf(chem,chem_pwdout,chem_pwout,outcm,cactiv,cnvwtm,zum,zdm,edtm,kpbli,zws,dhdt, &
           ...
           betam,autoconv,aeroevap,numc,itf,jtf,ktf,training, &
...
!      endif
            ...
            enddo
            enddo
      CALL neg_check(ktopm,'mid',ipn,dt,qcheck,outqm,outtm,outum,outvm,   &
                     outqcm,outcm,pretm,its,ite,kts,kte,itf,ktf,numc)
!!sms$ignore begin
!           do i=its,itf
...
!!sms$ignore end
      ...
      if(ideep.eq.1)then
      CALL CUP_gf(chem,chem_pwdout,chem_pwout,outc,cactiv,cnvwt,zu,zd,edt,kpbli,zws,dhdt,0,xmb,xmbm,xmbs,zo,   &
...
!      endif
            ...
            enddo
            enddo
      CALL neg_check(ktop,'deep',ipn,dt,qcheck,outq,outt,outu,outv,   &
...
!               endif
            ...
            DO nv=1,numc
             do k=kts,ktop(i)
                chem(i,k,nv)=max(1.e-16,chem(i,k,nv)+(outc(i,k,nv)+outcs(i,k,nv))*dt)
                !if(nv.eq.2)gdc(i,k,2)=(outc(i,k,nv)+outcs(i,k,nv))*86400.
                !if(nv.eq.6)gdc2(i,k,2)=(outc(i,k,nv)+outcs(i,k,nv))*86400.
                if(nv.eq.15)gdc(i,k,2)=(outc(i,k,nv)+outcs(i,k,nv))*86400.
                if(nv.eq.6)gdc2(i,k,2)=(outc(i,k,nv)+outcs(i,k,nv))*86400.
                !if(nv.eq.15)gdc(i,k,2)=chem_pwout(i,k,nv)
                !if(nv.eq.15)gdc2(i,k,2)=chem_pwdout(i,k,nv)
             enddo
            ENDDO
            endif
...
!       write(0,*)'ztm,ztq = ',ztm,ztq,hfm,qfm
   ...
   SUBROUTINE CUP_gf(chem,chem_pwdout,chem_pwout,outc,csum,cnvwt,zuo,zdo,edto,kpbl,zws,dhdt,imid,      &
   ...
   IMPLICIT NONE
     ...
     integer                                                           &
        ...
        numc,its,ite, jts,jte, kts,kte,ipr,jpr,imid
     integer, intent (in   )              ::                           &
  ...
  ! pre    = output precip
     ...
     real,    dimension (its:ite,kts:kte,numc)                              &
        ,intent (inout  )                   ::                           &
         chem,outc,chem_pwdout,chem_pwout
     real,    dimension (its:ite,kts:kte,numc)   ::                           &
         chem_cup,chem_up,chem_down,dellac,chem_c,chem_pw,chem_pwd
...
!
               ...
               !    endif
                   dz=zo_cup(i,K)-zo_cup(i,K-1)
                   trcc=chem_c(i,k,nv)/(1.+c0*dz)
                   chem_pw(i,k,nv)=c0*dz*trcc*zu(i,k)
...
!
                ...
                do ki=jmin(i)  ,1,-1
                  chem_down(i,ki,nv)=(chem_down(i,ki+1,nv)*zdo(i,ki+1)  &
                        ...
                        (zdo(i,ki+1)-.5*dd_massdetro(i,ki)+dd_massentro(i,ki))
                  chem_down(i,ki,nv)=chem_down(i,ki,nv)+pwdper(i,ki)*chem_pwav(i,nv) !lzhang pwdper(i,k)
...
!--------------------------------------------------------------------
   ...
   SUBROUTINE CUP_gf_sh(cnvwt,zuo,zws,xmb_out,xmbs2,zo,OUTQC,J,AAEQ,T,Q,Z1,       &
              chem,outc,pre,TN,QO,PO,P,OUTT,OUTQ,DTIME,PSUR,US,VS,                  &
   ...
   IMPLICIT NONE
     ...
     integer                                                           &
        ...
        numc,its,ite, jts,jte, kts,kte,ipr,jpr
     integer, intent (in   )              ::                           &
        j,ichoice
  !
  ! 
...
!
  ...
  ! edt     = epsilon
     ...
     integer                              ::                           &
       nv,nall,iedt,nens,nens3,ki,I,K,KK,iresult
     real                                 ::                           &
      day,dz,dzo,mbdt,radius,  &
