...
!           sfcdlw,tsflw,sfcemis,sfalb,swh,swhc,hlw,hlwc,ras,ideep_cnv, !
!           pre_rad,ldiag3d,lssav,lssav_cc,                             !
...
!     ideep_cnv- integer, 0=none; 1=SAS; 2=GF			   1    !
...
!     shal_cnv - logical, flag for calling shallow convection      1    ! replaced by ishal_cnv
!     ishal_cnv- integer, 0=none; 1=SAS; 2=GF; 3=SAS&GF            1    !
...
!  ====================    end of description    =====================  !
      ...
      subroutine gbphys                                                 &
!  ---  inputs:
     &    ( cactiv,ipn,im,ix,levs,lsoil,ntrac,ncld,ntoz,ntcw,           &
     &      nmtvr,nrcm,ko3,lonf,latg,jcap,num_p3d,num_p2d,              &
     &      kdt,lat,me,pl_coeff,nlons,ncw,flgmin,crtrh,cdmbgwd,         &
     &      ccwf,dlqf,clstp,cgwf,dtp,dtf,fhour,solhr,                   &
     &      slag,sdec,cdec,sinlat,coslat,pgr,ugrs,vgrs,                 &
     ...
     &      rann,prdout,poz,dpshc,hprime,xlon,xlat,                     &
     &      slope,shdmin,shdmax,snoalb,tg3,slmsk,vfrac,                 &
     &      vtype,stype,uustar,stress,oro,oro_uf,coszen,sfcdsw,sfcnsw,  &
     &      sfcdlw,tsflw,sfcemis,sfalb,swh,swhc,hlw,hlwc,ras,ideep_cnv, &
     &      pre_rad,ldiag3d,lssav,lssav_cc,                             &
...
!  ---  outputs:
     ...
     &      rqtk,dtdtr,evap2,hflx2,                                     &
     &      skip_cu_physics,skip_mp_physics, cubot, cutop,              &
     &      forcet,forceq,sdiaga,sdiagb,chem,numc)

!
      use machine ,   only : kind_phys
      use physcons,   only : con_cp, con_fvirt, con_g, con_rd, con_rv,  &
     &                       con_hvap, con_hfus, con_rerth, con_pi
      use namelist_def, only : n3dfercld, n2dfercld, n2dflxtvd
      use fimnamelist , only : n3dzhaocld,n3dflxtvd, n2dzhaocld,        &
     &                         n3dcldpdf, n2dcldpdf
...
!  ---  inputs:
      integer, intent(in) :: ix,   im,   levs, lsoil,   ntrac, ipn,     &
     ...
     &                       thermodyn_id, sfcpress_id
      ...
      integer, intent(in) :: nlons(im), ncw(2), nst_fcst, ishal_cnv,    &
     &                       numc,ideep_cnv
     ...
     &                       trans_trac, cal_pre,   gen_coord_hybrid
     ...
     &      ugrs, vgrs, tgrs, vvel, prsl, prslk, phil, swh,swhc,hlw,    &
     &      hlwc,forcet,forceq
      real(kind=kind_phys), dimension(ix,levs,11),    intent(inout) ::  &
     &      sdiaga,sdiagb
      real(kind=kind_phys), dimension(ix,levs,numc),intent(inout)::     &
     &       chem
...
! added for GF scheme
      real(kind=kind_phys) cubot,cutop
      integer, intent (inout) ::  cactiv(ix)
      LOGICAL skip_cu_physics,skip_mp_physics

      integer ipnglobal,its,mype
!JR Comment this out for efficiency--uncomment only when needed
!      call physics_get_ipn_its_mype_nomodule(ipnglobal,its,mype)
...
!     lprnt = me == 0
      ...
      ipr = 1
          !if(ipn.eq.430956)then
          !print*,'in gbphys vvel is ',vvel
          !endif
! jbao 2014 define variables which were defined in compns.f in gfs
...
!  --- ...  transfer soil moisture and temperature from global to local variables
      ...
      if(skip_cu_physics)then
         ktop(1) = cutop
         kbot(1) = cubot
         RAINC(1) = FRAIN * cnvprcp(1) ! FRAIN * RAIN1(1)
         rain1(1)= FRAIN * cnvprcp(1) ! 0. ! dtp*bengsh(1)
         RAINL(1)= 0.
      endif
         RAINCS(1)= 0.

!tgs - old GFS veg. types
...
!       sigmaf(i)   = max( vfrac(i),0.3 )
        ...
        if (islmsk(i) == 2) then
          ...
          !tgs - for MODIS
         if(alt_land) then
          vegtype(i) = 15   ! MODIS - snow and ice
         else
          vegtype(i)  = 13
         endif
...
!      if (ipn == 3255)then
!         print*,'after dcyc2t3'
!         print *,'adjsfcdsw,adjsfcnsw,adjsfcdlw,adjsfculw,xmu,xcosz'
!         print *,adjsfcdsw,adjsfcnsw,adjsfcdlw,adjsfculw,xmu,xcosz
!      endif
...
! jbao 2014 these are set to 0 in 2012
          gflx=0.0
! jbao 2014 these are set to 0 in 2012
...
!  ---  outputs:
     &         qss, gflx, cmm, chh, evap, hflx, evap2, hflx2, ep1d)
...
!  ---  outputs:
     &       qss,cmm,chh,gflx,evap,hflx,evap2,hflx2,ep1d                &
     &     )
...
!
                                                     ! noah lsm call
!       if (ipn == 3255.and.iter == 1)then
!         print*,'before sfc_drv, ipn=',ipn
!         print*,'im',im    
!         print*,'lsoil',lsoil
!         print*,'pgr',pgr
!         print*,'ugrs',ugrs
...
!         print*,'sigmaf',sigmaf
!         print*,'sfcemis',sfcemis
!         print*,'gsfcdlw',gsfcdlw
!         print*,'adjsfcdsw',adjsfcdsw
!         print*,'adjsfcnsw',adjsfcnsw
!         print*,'dtf',dtf
!         print*,'tg3',tg3
!         print*,'cd',cd
!         print*,'cdq',cdq
!         print*,'prsl(1,1)', prsl(1,1)
!         print*,'prsl', prsl(1,1)
!         print*,'work3',work3
!         print*,'zlvl',zlvl
!         print*,'islmsk',islmsk
!         print*,'phy_f2d',phy_f2d(1,1)
!         print*,'slopetyp',slopetyp
!         print*,'shdmin', shdmin
!         print*,'shdmax',shdmax
!         print*,'snoalb',snoalb
!         print*,'sfalb',sfalb
!         print*,'flag_iter',flag_iter
!         print*,'flag_guess',flag_guess
!         print*,'weasd', weasd
!         print*,'snwdph',snwdph
!         print*,'tsea',tsea
!         print*,'tprcp',tprcp
...
!  ---  in/outs:
     ...
     &       canopy,trans,tsurf,                                        &
!  ---  outputs:
     &       sncovr,qss,gflx,drain,evap,hflx,evap2,hflx2,ep1d,runof,    &
     &       cmm,chh,evbs,evcw,sbsno,snowc,soilm,snohf,                 &
     &       smcwlt2,smcref2,zorl                                       &
     &     )


!      if (ipn.eq.3255.and.iter == 2)then
!         print*,'after sfc_drv, ipn=',ipn
!         print*,'im',im    
!         print*,'lsoil',lsoil
!         print*,'pgr',pgr
!         print*,'ugrs',ugrs
...
!  ---  outputs:
     &       snwdph,qss,snowmt,gflx,cmm,chh,evap,hflx,evap2,hflx2       &
     &     )
...
!     if (lprnt)  print *,' dtdtb=',dtdt(ipr,:)
      ...
      if((qgrs(1,1,1).lt.1.e-4).and.(evap(1).lt.0.))then
       evap(1)=0.  ! jbao positive definite check for excessive downwrad
...
!jbao 2014 test no sl     call rayleigh_damp(im, ix, im, levs, dvdt, dudt, dtdt,
! jbao 2014 test no sl     &                   ugrs,   vgrs, dtp, con_cp)
...
!
      ...
      if (ideep_cnv==1) then		! SAS deep convection
     ...
     &                 cnvw,cnvc)
      elseif (ideep_cnv==2) then	! gf deep convection
          call GFDRV(cactiv,ipn,ntrac,tottracer,lonf,IX,levs,DTP
     &      ,forcet,forceq,phil,RAIN1
     &      ,GQ0,GT0,cld1d,gu0,gv0,tGRS,VVEL,QGRS,prsl,pgr
     &      ,kbot,ktop,kcnv,SLMSK,hflx,evap,clw
     &      ,hpbl,ud_mf,dd_mf,dt_mf,sdiaga,sdiagb,cnvw,cnvc,chem,numc
     &      ,ishal_cnv)
      endif
...
!           dqdt(i,k,1) = gq0(i,k,1)
      ...
      if (ishal_cnv==1 .or. ishal_cnv==3) then ! 0=none; 1=SAS; 2=GF; 3=SAS&GF
...
! jbao 2014 orig
! jbao 2014 orig  &  rainp, rhc, psautco_l, prautco_l,evpco, wminco,    &
! jbao 2014 
     &          rainp, rhc, psautco_l(1), prautco_l(1), evpco, wminco,  &
     &                lprnt, ipr)
...
!  1/1/2015 - Stan B - add condition for surface temp < 1 deg C for snow
!  identification in setting srflag (snow/rain flag for precipitation)
          if (t850(i) <= 273.16. and. gt0(i,1)< 274.16) then
