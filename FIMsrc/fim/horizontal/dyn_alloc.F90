...
contains
  ...
  subroutine dyn_alloc
    use module_control,only: nvlp1,npp,nip,nabl,ntra,ntrb,nd,nvlsig
...
!$OMP PARALLEL DO
    do ipn=ims,ime
      rsds(ipn) = 0.	! replace inf by 0 to prevent inf in the initial output when physics is called yet
      conv_act(ipn)=0
      rlds(ipn) = 0.
      rsus(ipn) = 0.
      rlus(ipn) = 0. 
