...
subroutine run ()
  use module_chem_driver_gocart           ,only: chem_run300=>gocart
  use module_chem_driver_gocartracm       ,only: chem_run301=>racmgocart
  use module_chem_driver_RACMSOAVBS       ,only: chem_run108=>racmsoavbs
...
!sms$serial end
        ...
        if (chem_opt > 0) then
          ...
          if(chem_opt == 300) call chem_run300 (its)     
          if(chem_opt == 301) call chem_run301 (its)     ! chemistry run method.  
          if(chem_opt == 108) call chem_run108 (its)     ! chemistry run method.  
        end if
...
! Write a restart file if it is time
...
!     if (its.gt.____) PrintIpnDiag=____
      ...
      print "('its=',i7,4x,a,f8.2,a,f9.3)", its, ArchvTimeUnit, its2time(its), ' time(s)=', wall2-wall1
    end do ! time step loop
