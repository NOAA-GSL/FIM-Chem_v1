module module_fim_dyn_init
  ...
  use fimnamelist     ,only: nvl,TotalTime,ArchvIntvl,ArchvTimeUnit,itsStart,  &
                             ...
                             dt_reducer_numerator,ptop,dt_reducer_denominator, &
...
!SMS$SERIAL (<nvlsig,ptop,OUT> : default=ignore)  BEGIN
! "82" is a magic unit that might have to be byte-swapped
      ...
      if (nvlsig == 64) then
          ptop = 50.
      elseif (nvlsig == 42) then
          ptop = 100.
      else
        print *,' wrong nvlsig =',nvlsig
        stop
      end if
!SMS$SERIAL END
