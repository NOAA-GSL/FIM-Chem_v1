MODULE MODULE_CHEM_DRIVER_GOCARTRACM
   IMPLICIT NONE

#include <gptl.inc>

CONTAINS
     subroutine racmgocart(ktau)
!
...
! TBH:  Ignore these so PPP does not have to translate them
  ...
  USE module_initial_chem_namelists,only:p_qc,chem_opt,biomass_burn_opt,seas_opt,dust_opt,&
                            ...
                            p_hket,p_api,p_lim,p_dien,p_macr,p_ho,p_ho2,p_dms,p_msa,p_ol2,&
! aerosols
...
if(call_plume == 1 )  then
          call plumerise_driver (ktau,dtstep,num_chem,num_moist,num_ebu, &
           ...
           mean_fct_agsv,mean_fct_aggr,firesize_agtf,firesize_agef,      &
           firesize_agsv,firesize_aggr,'GOCARTRACM','BIOMASSB', t_phy,   &
...
!                         +ebu(i,k,j,p_ebu_dms)*factor2
 ...
 !            nlong=xlong(i,j)
 !        endif
 !    call get_diurnal_rate(ktau,dt,current_month,current_day,current_year,xlat(i,j),nlong,rate)
!lzhang
...
!lzhang
     !if(moist(i,k,j,p_qr) > 1.e-18 .or. raincv_b(i,j) > 0.) then
     !if( raincv_b(i,j) > 0.) then
!        chem(i,k,j,p_iso)  = chem(i,k,j,p_iso)
!        emis_ant(i,k,j,p_e_iso)=0.0
     !else
         if (xlong(i,j)>180) then
             nlong=xlong(i,j)-360.
         else
...
END MODULE MODULE_CHEM_DRIVER_GOCARTRACM
