...
!  use module_chem_variables,          only: ext_cof, asymp, extlw_cof
    use module_variables,          only: ext_cof,sscal, asymp, extlw_cof, conv_act
...
!SMS$DISTRIBUTE END
    ...
    read (unitno, err=90) facwf2d, t2m2d, q2m2d, conv_act
...
! readarr64_r tells SMS to do the right thing.
    do k=1,gis_phy%levs
      ...
      call readarr64_r (sscal_loc,   nbands, unitno)
      do n=1,nbands
        do ipn=1,nip
          ext_cof(k,ipn,n) = ext_cof_loc(ipn,n)
          sscal(k,ipn,n) = sscal_loc(ipn,n)
        end do
      end do
...
!SMS$DISTRIBUTE END
    ...
    write (unitno, err=90) facwf2d, t2m2d, q2m2d, conv_act
