...
!*********************************************************************
  ...
  use module_control, only: nip, itsdfi, dospptdt, ntra
  use fimnamelist,    only: nvl, digifilt, ishal_cnv, ideep_cnv, sppt
...
! jbao orig USE resol_def,		ONLY : levs,levp1
    ...
    USE module_variables,	ONLY : diaga,diagb,tr3d,conv_act
    USE module_constants,	ONLY : prox,nprox
    USE findmaxmin1
    use module_wrf_control, only: nbands,num_chem
    USE module_wrf_variables, ONLY:phys3dwrf,phys2dwrf
    USE module_wrfphysvars, ONLY: RTHRATENSW,RTHRATENLW,gd_cloud,gd_cldfr,gd_cloud2,snowh
    USE mersenne_twister
!JR Moved namelist_def here from gloopr to enable calling rad_initialize
    use namelist_def, only: crick_proof, ccnorm, norad_precip, sashal, ras, &
                            ldiag3d, lssav
...
!JR to do_physics_one_step are also visible here
    subroutine column_physics (ipn)
      ...
      real(kind=kind_phys) :: cldcov(ix,nvl)
...
!
! do transport of chem variables in gfconv
!
      real(kind=kind_rad) :: chem(ix,nvl,num_chem)
!
...
!     real(kind=kind_phys), save :: psautco(2) = (/6.0E-4,3.0E-4/) !  update from gfs 2014 input
      ...
      !----------------------------------------------------------------------
        ...
        do k=1,nvl
          ...
          qg1(1,k,2) = gfs_cld(ipn,k)+ gd_cloud(1,k,ipn)
          if (skip_cu_physics) then
            !        qg1  (1,k,2) = gfs_cld(ipn,k) +(diaga(k,ipn)+diagb(k,ipn))/float(CallRadiation)
            qg1(1,k,2) = gfs_cld(ipn,k)   !+ sdiaga(k,ipn,1) + sdiagb(k,ipn,1) 
!            diaga(k,ipn) = 0.
!            diagb(k,ipn) = 0.
          end if
          prsl(1,k) = prsl_s(1,k)
        ...
        if (chem_opt > 0 .and. aer_ra_feedback > 0 ) then
          sscal_rad(:,:) = sscal(:,ipn,:)
          asymp_rad(:,:) = asymp(:,ipn,:)
...
! jbao 2014
        ...
        CALL GLOOPR                                           &
           ...
           qg(1,1,1),qg1(1,1,1),                               &
           yyyymmddhhmm,sscal_rad,asymp_rad,ext_cof_rad,       &
           extlw_cof_rad,cldcov)
        ret = gptlstop ('gloopr')
        
!JR Copy things manually to avoid copyin/copyout
!JR If any of these things are input only, the copy can be eliminated
        do k=1,nvl-1  
        gd_cldfr(1,k,ipn)=cldcov(1,k)
        enddo
...
! gg : advective forcing for GF
      ...
      if(kdt > 1 ) then
       do nv=1,num_chem
         do k=1,nvl
           chem(1,k,nv)=tr3d(k,ipn,ntra+nv)
         enddo
       enddo
! change this when reinitializing..... kludge for now!!
      else
       chem(:,:,:)=1.
      endif
...
!  ---  outputs:
           ...
           forcet,forceq,sdiaga,sdiagb,chem,num_chem)
      ret = gptlstop ('gbphys')
!JR Copy things manually to avoid copyin/copyout.
...
!
     do k=1,nvl-1
        diaga(k,ipn) = sdiaga(1,k,2)
        diagb(k,ipn) = sdiagb(1,k,2)
       gd_cloud(1,k,ipn) =  sdiaga(1,k,1)+sdiagb(1,k,1)
!       diagb(k,ipn) = sdiagb(1,k)
     enddo
!  endif
...
!          taperoff = 1.0
          ...
          if(kdt > 1 .and. num_chem > 0 .and. chem_conv_tr ==1 ) then
            do nv=1,num_chem
              tr3d(k,ipn,ntra+nv)=chem(1,k,nv)
            enddo
          endif
      end do
