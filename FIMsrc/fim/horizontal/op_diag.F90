module module_op_diag
  ...
  use module_control,         only: nvlsig,nvlp1,nip,ntra,ntrb,	&
                                    ...
                                    pres_pa
  use fimnamelist,            only: nvl,glvl,curve,yyyymmddhhmm,gz_smooth,	&
                                    ArchvTimeUnit,PrintIpnDiag, pure_sig,  &
                                    enkfio_out, TimingBarriers
...
! Obtain isobaric non-chem scalar fields
      ...
      do k=1,nvlp
        if (g3p(k,ipn,1).ne.spval_p) then
          g3p(k,ipn,3) = fldp(k,1)			! rel.humidity
