module fimnamelist
  ...
  integer,private,parameter :: number_of_namelists = 17       !
...
! ?*. stands for spatial: i - icos, g - gaussian, s - spectral
  ...
  integer        :: dtratio                = 4         ! tracer B / tracer A timestep ratio
  ...
  integer        :: flux_conserv_schm      = 1         ! Use 1: FCT, 2: flux_limiter 
  integer, parameter :: janjic             = 0         ! 1: partial Janjic; 2: full Janjic
  logical        :: hiOrderFluxComp        = .false.   ! Use high order flux computation scheme
  ...
  real           :: ptop                   = 50.       ! pres at top of model domain (Pa)
  ...
  real           :: tfiltwin               = 0.        ! length of digital filter window (secs)
  real           :: thktop                 = 50.       ! min.thickness (Pa) of uppermost layer
  logical        :: UpdateSST              = .false.   ! Update SST field with interpolated monthly SST and Sea Ice Fraction?
  real           :: veldff_bkgnd           = 1.        ! diffusion velocity (=diffusion/meshsz)
